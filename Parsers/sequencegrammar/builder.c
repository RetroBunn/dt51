...
"actions" of the parser which is defined in sgrammar.y. 
In particular, the function buildtree() defined here is
called by the code generated from sgrammar.y 
...
void filedumpGTrie(GTrie *Gt, FILE *outstr, const char *nameextension)
{
  ...
  fprintf(outstr,"/* It is generated with the sgrammar word parser software           */\n");
  fprintf(outstr,"/* in the new version including sequence analysis                 */\n");
  fprintf(outstr,"/* Written by Reiner Wilhelms-Tricarico  @ FONIX                  */\n\n");

  //  fprintf(outstr,"#ifndef   SGRAMMAR_IN_DECTALK /* automatic include */\n");
  //  fprintf(outstr,"#define  SGRAMMAR_IN_DECTALK  /* compact version of includes */  \n");
  //  fprintf(outstr,"#include \"seqanalyse.h\" \n\n");
  //  fprintf(outstr,"#endif  /* SGRAMMAR_IN_DECTALK */\n\n");
...
void 	validate_operator(ASCIILETTER *name, U16 *code, U16 *parameter)
{
  ...
  *parameter=0;
  ...
  for (i=0; i<NUMBER_OF_SGRAMMAR_OPERATORS; i++)
    {
...
/* 
   progbuild is called in the parsing stage (see sgrammar.y) for all 
...
void make_association(char *name, char *name2)
{
    ...
    if (ret > 0)
    {
	fprintf(stdout,"ERR: name %s was already defined in AssociationDict\n",name);
    }
}
...
void progprint(FILE *fid, FILE *fidh, seqprogtype type, char *name, int index, gnodetype gtype)
{
	...
	case SQNC_PARRIGHT:
	    ...
	    break;
	case SQNC_CITE:
	    fprintf(fid,"   v = defseq_%s(N,P,0,0); \n",name);
	...
	case SQNC_ACTION:
	    ...
	    break;
	case SQNC_CITELOOPED:
	    fprintf(fid,"   v = defseq_%s(N,P,%c,%c); /* looped */ \n",&name[2],name[0],name[1]);
	    fprintf(fid,"   if (!v) return 0;\n\n");
	    break;
    }
}
...
/* -----------------------------------------------------------------
   ...
   (yacc or bison generated-) parser. In the file sgrammar.y, one can
   see this subroutine called inside of various action brackets {}.
   The yacc or bison generated program sgrammar.tab.c results then
...
BOOL  buildtree(gnodetype how, ASCIILETTER *name, int val, int valb)
{
 ...
 // The others are considered.
	...
	inf.counter=val;       // first of the two numbers [x,y] - see sgrammar.y
	...
	// note that symlook, called by lex (sgrammar.l) starts counting at 1.
	// Hence, if yacc (see sgrammar.y) returns val, we have to address via val-1.
	...
	{
	  FILE *outstr;
	  outstr = fopen("sgrammarsequence.h","w");
	  if (outstr == NULL)
	    {
		printf("buildtree (builder.c) could not open output file sgrammarsequence.h \n");
		perror(name);
		exit(1);
	    }
