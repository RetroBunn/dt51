...
SEQUENCEDECL:    DEF ':' NAME '=' FILENAME { make_association($3->name,$5); 
              ...
              |  SEQ ':' NAME ':'  { char tmpstr[64];
	                             sprintf(tmpstr,"%s%i",$3->name,lex_line());
		                     progbuild(SQNC_NEWSEQ,tmpstr);} SEQUENCE {progbuild(SQNC_ENDSEQ,0);}
              |  SEQ  {char tmpstr[64];
...
SEQOTHER:       '%' BOUND '%' {progbuild(SQNC_ACTION,$2);}
             |  '&' NAME '[' INTEGER   ','   INTEGER ']'   {char tmpstr[64];  /* put in temporary string and send to progbuild */
							    sprintf(tmpstr,"%c%c%s",$4+48,$6+48,$2->name);
