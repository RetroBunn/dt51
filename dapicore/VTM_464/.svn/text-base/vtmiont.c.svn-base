...
/* *****************************************************************/
  ...
  {
	  pVtm_t = (VTM_T*)calloc(1, sizeof(VTM_T));
...
*/
	...
	else if(pVtm_t->parambuff[OUT_A2+1] == 3100)
	{
		...
		if(pVtm_t->parambuff[OUT_PH+1] == USP_HX)
			pVtm_t->llframe.NA2F = 30;
		if( pVtm_t->parambuff[OUT_PH+1] == GRP_H)
...
//		}
  ...
  //llframe.NATV=0;
	...
	if(DT_DBG(PH_DBG,0x040))
	{
			...
			printf(" an  %d ",pVtm_t->parambuff[OUT_AN+1]);
		 ...
		 DWORD NextPhone)
{
  ...
  PKSD_T pKsd_t = phTTS->pKernelShareData;
	...
	{
		pTemp = (short*)calloc(100, sizeof(short));
