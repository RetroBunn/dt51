...
#define DICT_LANG pKsd_t->lang_curr
#else
...
#ifdef USE_NAND
#define DICT_INDEX       ((U32 *)pKsd_t->fdic_index)
#define FDICT_INDEX      ((U32 *)pKsd_t->foreigndic_index)
#define UDICT_INDEX      ((U32 *)pKsd_t->udic_index)
#define ADICT_INDEX      ((U32 *)pKsd_t->adic_index)

#define DICT_DATA       (((pKsd_t->fdic_data)))
#define FDICT_DATA      (((pKsd_t->foreigndic_data)))
#define UDICT_DATA      (((pKsd_t->udic_data)))
#define ADICT_DATA      (((pKsd_t->adic_data)))

#define DICT_ACCESS(x)       ((struct dic_entry *)(&(pKsd_t->fdic_data[((U32 *)pKsd_t->fdic_index)[(x)]])))
#define FDICT_ACCESS(x)      ((struct dic_entry *)(&(pKsd_t->foreigndic_data[((U32 *)pKsd_t->foreigndic_index)[(x)]])))
#define UDICT_ACCESS(x)      ((struct dic_entry *)(&(pKsd_t->udic_data[((U32 *)pKsd_t->udic_index)[(x)]])))
#define ADICT_ACCESS(x)      ((struct dic_entry *)(&(pKsd_t->adic_data[((U32 *)pKsd_t->adic_index)[(x)]])))

#define DICT_ENTRY      pKsd_t->fdic_entries
#define FDICT_ENTRY     pKsd_t->foreigndic_entries
#define UDICT_ENTRY     pKsd_t->udic_entries
#define ADICT_ENTRY     pKsd_t->adic_entries

#define DICT_BYTES      pKsd_t->fdic_bytes
#define FDICT_BYTES     pKsd_t->foreigndic_bytes
#define UDICT_BYTES     pKsd_t->udic_bytes
#define ADICT_BYTES     pKsd_t->adic_bytes
#else
...
#define UDICT_INDEX_LVALUE      (pKsd_t->udic_index[DICT_LANG])
...
#ifdef USE_NAND
#define DICT_FC_ACCESS(x)       (pKsd_t->fdic_fc_entry[x])
#else
