...
void ls_rule_lts(LPTTS_HANDLE_T phTTS,LETTER *llp, LETTER *rlp, FLAG ContD) 
{
	...
	memset(precednor,0,80);
	memset(phrase,0,80);
	memset(phrasenor,0,80);
	memset(suivant,0,80);
	memset(phon,0,80);
...
static void Element_ClSeq (short PtChCl, BYTE *PremPhon, short Inc, short *LgECl)
{
	  BYTE  *ptc;	   /* pointor on Phon or PhonT */
	  FLAG	Trouve;	   /* true if found the word in the class */
	  short	LgMotCrt=-1;  /* length of the word in the class */
	  short	i;
...
static short VCG (PLTS_T pLts_t, TypTamp *Tampon, short PtInst)
{
	  ...
	  short	NbTerm=0;     /* length of the part of context */
	  short	PtContx=0;    /* index on EnsOuCh for part of context examination */
	  short	NumClas=0;    /* class number  */
	  short	Longueur=-1;   /* length of the part of context */
	  FLAG	Corect;
...
static short VCD (PLTS_T pLts_t, TypTamp *Tampon, short PtInst, short LgPG)
{
	  ...
	  short	Longueur=-1;  /* length (nb char.) of current term */
