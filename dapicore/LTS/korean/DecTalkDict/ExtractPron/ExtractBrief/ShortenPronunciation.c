...
void DumpIndexList(llistPtr pList, char *sEndString)
{
   ...
   for (pLink=pList; pLink; pLink=pLink->pNext)
   {
      ...
      printf("%d%s", pNode->iID, pLink->pNext ? ", " : sEndString);
   }
}
...
void DumpDGraph(DGraphPtr pDGraph, char *sMessage)
{
   ...
   if (sMessage && sMessage[0])
      printf("%s\n", sMessage);
   printf("%d Nodes\n", pDGraph->iNumNodes);
   printf("%d Initial Nodes: ", ListSize(pDGraph->InitNode.pNext));
   DumpIndexList(pDGraph->InitNode.pNext, "\n");
   printf("%d Final Nodes:   ", ListSize(pDGraph->EndNode.pPrev));
...
BOOL SameDGraphs(DGraphPtr pDGraph1, DGraphPtr pDGraph2)
{
   ...
   for (iNode=0; iNode<pDGraph1->iNumNodes; iNode++)
   {
      ...
      {
         printf("Wrong ID#s: For node %d, IDs are %d and %d\n", iNode, pDNode1->iID, pDNode2->iID);
         return FALSE;
      }
      ...
      //{
      //   printf("Disjunction mismatch for node %d\n", iNode);
      //   return FALSE;
      //}
      if (pDNode1->bOptional != pDNode2->bOptional)
      {
         printf("Optional mismatch for node %d\n", iNode);
         return FALSE;
      }
      if (!SameIDLists(pDNode1->pPrev, pDNode2->pPrev))
      {
         printf("Different prev lists for node %d\n", iNode);
         return FALSE;
      }
      if (!SameIDLists(pDNode1->pNext, pDNode2->pNext))
      {
         printf("Different next lists for node %d\n", iNode);
         return FALSE;
      }
      if (!SameSymbolLists(pDNode1->pSymbols, pDNode2->pSymbols))
      {
         printf("Different symbol lists for node %d\n", iNode);
         return FALSE;
      }
   }  /* for iNode */
...
DGraphPtr CopyDGraph(DGraphPtr pDGraph, LinkPoolPtr pLinkPool)
{
   ...
   //   printf("Uh-oh! Copy not the same as the original!\n");
#endif
   return pCopy;
}  /* CopyDGraph */
...
#if 0
//EXPERIMENTAL: Seems to be exponential in nature, so don't use it.
SplitNodes(DGraphPtr pDGraph, LinkPoolPtr pLinkPool)
{
   DGraphPtr pBestGraph=NULL;     /* Best DGraph found from all attempts at splitting nodes. */
   SDWORD   iBestNumSymbols;      /* Number of symbols in pBestSymbolList */
   SDWORD   iBestNumChars;        /* Number of characters in pBestSymbolList's string */
   llistPtr pSymbolCopy;          /* Temporary list to use to hold a copy */
   char *sPronunciation;          /* Temporary string to see how long a pronunciation is */
   SDWORD iNumSymbols, iNumChars;
   SDWORD iNode;
   DNodePtr pNode, pPrevNode, pNodeCopy, pPrevNodeCopy, pNextNode, pNextNodeCopy, pSplitNode;
   llistPtr pLink, pPrevLink, pNextLink;
   DGraphPtr pDGraphCopy;
   static iDepth=0;
   static iMaxDepth;
   static iCalls=0;
   iCalls++;

   /* Keep track of how deep the recursion is.  If we go too deep, then
      return. */
   if (iCalls%1000==0)/////
      printf("%d calls (depth=%d, numnodes=%d)   \r",iCalls,iDepth,pDGraph->iNumNodes);
   if (iDepth==0)
   {
      /* When first calling the routine, set the maximum recursion depth
         to twice the number of nodes in the graph (which should hopefully
         be a very safe upper bound, i.e., we hopefully won't have to
         split every node in the graph in order to collapse the graph).
         Then if we go deeper than that, then we will know we're in an
         infinte loop and will be able to recover from it. */
      iMaxDepth = pDGraph->iNumNodes * 2;
      iCalls = 0;
   }
   iDepth++;
   //printf("%*s%*s\n",iDepth+1,"*",iMaxDepth-iDepth+1,"|");
   if (iDepth > iMaxDepth || iCalls > 100000)
   {
      /* Exceeded maximum depth, so we're probably in an infinite loop */
      FreeDGraph(pDGraph, pLinkPool);
#ifdef DEBUG_DGRAPH
      printf("    [Avoiding infinite loop.  Returning NULL.]\n");
#endif
      iDepth--;
      return FALSE;
   }

   /* We have a complex graph requiring at least one node's symbols to be repeated
      in order to build a string. For example:
        Init---a---End
            \   \  /
             \---b/
      can be written in 3 ways: "a|b|ab", "a|[a]b", or "a[b]|b" (not counting
      equivalent permutations like "ab|a|b").  Depending on the number of symbols
      in nodes a and b, one way might result in a shorter string that another.
      Since we don't know in advance, we can either (a) try all possibilities
      and take the shortest, or (b) use a "greedy" approach and live without optimality. */
   /* (a) Here we try all possibilities: */
   /* 5. Find all nodes with (i) more than 1 prev or next node, and (ii) whose
         next and prev nodes are connected to more than just this node (or else
         Rule 2 will re-merge it again). */
   for (iNode=0; iNode < pDGraph->iNumNodes; iNode++)
   {
      pNode = pDGraph->ppNodes[iNode];
      if (iDepth==1)
      ...
      if (ListSize(pNode->pPrev)>1)
      {
         ...
         {
            pPrevNode = (DNodePtr)pPrevLink->p;
            /* Occasionally there is a single node "between" pNode and an
               qualifying previous node (i.e., one with multiple "next" nodes)
               that would otherwise prevent us from finding a split (see
               Randy Wilson's Log Book V, page 133, for an example).
               So if there's a "one-in, one-out" node before pNode,
               go to ITS previous node to see about a split. */
            if (ListSize(pPrevNode->pPrev)==1 && ListSize(pPrevNode->pNext)==1)
               pPrevNode = (DNodePtr)pPrevNode->pPrev->p;
            if (ListSize(pPrevNode->pNext)!=1)
            {
               /* pPrevNode has something besides pNode in its pNext list,
                  so we can try splitting pNode along its pPrev list without
                  having the two nodes recombined by Rule 2 (and thus having an infinite loop). */
               break; /* Exit "for pPrevLink" loop */
            }
         }
         if (pPrevLink) /* i.e., if we can split pNode on its pPrev side */
         {
            llistPtr pStart;
            /* If there are only two prev nodes, start with second one so that
               we don't try splitting the prev nodes into the same two partitions
               twice (e.g., if a & b both connect to c, then doing a-c1, b-c2
               makes it unnecessary to do b-c1, a-c2, since that's the same thing.
               However, if a,b,c all connect do d, we DO need to do
                 a-d1, b/c-d2; b-d1, a/c-d2; and c-d1, a/b-d2.  If that makes sense.) */
            if (ListSize(pNode->pPrev)>2)
               pStart = pNode->pPrev;
            else pStart = pNode->pPrev->pNext;

            /* Try splitting pNode so that the copy connects to each one
               of its previous nodes, and the original connects to everything else. */
            for (pPrevLink = pStart; pPrevLink; pPrevLink = pPrevLink->pNext)
            {
               pPrevNode = (DNodePtr)pPrevLink->p;
               
               /*** Make a copy of pDGraph to modify */
               pDGraphCopy = CopyDGraph(pDGraph, pLinkPool);
               
               /* Get pointers to corresponding nodes in the COPY of DGraph */
               pNodeCopy = pDGraphCopy->ppNodes[pNode->iID];
               if (pPrevNode->iID==INIT_NODE_ID)
                  pPrevNodeCopy = &pDGraphCopy->InitNode;
               else pPrevNodeCopy = pDGraphCopy->ppNodes[pPrevNode->iID];

               /* Remove link between pPrevNodeCopy and pNodeCopy, since
                  pPrevNodeCopy will instead connect to pSplitNode. */
               RemovePointerFromList(pNodeCopy, &pPrevNodeCopy->pNext, pLinkPool);
               RemovePointerFromList(pPrevNodeCopy, &pNodeCopy->pPrev, pLinkPool);

               /* pDGraphCopy had one additional node allocated, so use that. */
               pSplitNode = &pDGraphCopy->pNodeBuf[pDGraphCopy->iNumNodes];
               pSplitNode->iID = pDGraphCopy->iNumNodes;
               pDGraphCopy->ppNodes[pSplitNode->iID] = pSplitNode;
               pDGraphCopy->iNumNodes++;
               pSplitNode->bOptional    = pNodeCopy->bOptional;
               pSplitNode->pDisjunction = CopyListOfLists(pNodeCopy->pDisjunction, pLinkPool);
               pSplitNode->pSymbols     = CopyList(pNodeCopy->pSymbols, pLinkPool);

               /* Connect pSplitNode to all of pNodeCopy's pNext nodes. */
               pSplitNode->pNext        = CopyList(pNodeCopy->pNext, pLinkPool);
               for (pLink=pSplitNode->pNext; pLink; pLink=pLink->pNext)
               {
                  pNextNode = (DNodePtr)pLink->p;
                  InsertLink(pSplitNode, &pNextNode->pPrev, pLinkPool);
               }

               /* Connect pSplitNode to pPrevNodeCopy */
               pSplitNode->pPrev = NULL;
               ConnectDNodes(pPrevNodeCopy, pSplitNode, pLinkPool);
               
               /* Now Recurse into CollapseDGraph() and see what symbol list
                  we get after having split this node. */
               if (CollapseDGraph(pDGraphCopy, pLinkPool)==0)
               {
                  iNumSymbols    = ListSize(pDGraphCopy->ppNodes[0]->pSymbols);
                  pSymbolCopy    = CopyList(pDGraphCopy->ppNodes[0]->pSymbols, pLinkPool);
                  sPronunciation = SymbolsToString(pSymbolCopy, pLinkPool);
                  iNumChars      = FNX_STRLEN(sPronunciation);
                  if (pBestGraph==NULL || iNumSymbols < iBestNumSymbols ||
                      (iNumSymbols==iBestNumSymbols && iNumChars < iBestNumChars))
                  {
                     FreeDGraph(pBestGraph, pLinkPool);
                     pBestGraph = pDGraphCopy;
                     iBestNumSymbols = iNumSymbols;
                     iBestNumChars = iNumChars;
                  }
                  else FreeDGraph(pDGraphCopy, pLinkPool);
                  FNX_FREE(sPronunciation);
               }
            }  /* for each previous node */
         }  /* if can split node on previous nodes */
      }  /* if #prev nodes > 1 */
      ...
      if (ListSize(pNode->pNext)>1)
      {
         for (pNextLink = pNode->pNext; pNextLink; pNextLink = pNextLink->pNext)
         {
            ...
            /* If pNextNode is a "one-in, one-out" node, then go to ITS next node. */
            if (ListSize(pNextNode->pPrev)==1 && ListSize(pNextNode->pNext)==1)
               pNextNode = (DNodePtr)pNextNode->pPrev->p;

            if (ListSize(pNextNode->pPrev)!=1)
            {
               /* pNextNode has something besides pNode in its pPrev list,
                  so we can try splitting pNode along its pNext list without
                  having the two nodes recombined by Rule 2 (and thus having an infinite loop). */
               break; /* Exit "for pNextLink" loop */
            }
         }
         if (pNextLink) /* i.e., if we can split pNode on its pNext side */
         {
            llistPtr pStart;
            if (ListSize(pNode->pNext)>2)
               pStart = pNode->pNext;
            else pStart = pNode->pNext->pNext;

            /* Try splitting pNode so that the copy connects to each one
               of its next nodes, and the original connects to everything else. */
            for (pNextLink = pStart; pNextLink; pNextLink = pNextLink->pNext)
            {
               pNextNode = (DNodePtr)pNextLink->p;
               ...
               pNodeCopy = pDGraphCopy->ppNodes[pNode->iID];
               if (pNextNode->iID==END_NODE_ID)
                  pNextNodeCopy = &pDGraphCopy->EndNode;
               else pNextNodeCopy = pDGraphCopy->ppNodes[pNextNode->iID];

               /* Remove link between pNextNodeCopy and pNodeCopy, since
                  pNextNodeCopy will instead connect to pSplitNode. */
               RemovePointerFromList(pNodeCopy, &pNextNodeCopy->pPrev, pLinkPool);
               RemovePointerFromList(pNextNodeCopy, &pNodeCopy->pNext, pLinkPool);
               ...
               /* Connect pSplitNode to all of pNodeCopy's pPrev nodes. */
               pSplitNode->pPrev        = CopyList(pNodeCopy->pPrev, pLinkPool);
               for (pLink=pSplitNode->pPrev; pLink; pLink=pLink->pNext)
               {
                  pPrevNode = (DNodePtr)pLink->p;
                  InsertLink(pSplitNode, &pPrevNode->pNext, pLinkPool);
               }

               /* Connect pSplitNode to pNextNodeCopy */
               pSplitNode->pNext = NULL;
               ConnectDNodes(pSplitNode, pNextNodeCopy, pLinkPool);
         ...
         }  /* if can split node on next nodes */
      }  /* if #next nodes > 1 */
   }  /* for iNode */

   if (pBestGraph != pDGraph)
   {
      /* Swap the memory contents of pDGraph and pBestGraph (including pointers)
         so we can free up the stuff that used to be in pDGraph and replace it
         with the new stuff in pBestGraph without messing up the pointer to
         pDGraph used by the calling routine. */
      DGraph temp;
      memcpy(&temp, pDGraph, sizeof(DGraph));
      memcpy(pDGraph, pBestGraph, sizeof(DGraph));
      memcpy(pBestGraph, &temp, sizeof(DGraph));
      FreeDGraph(pBestGraph, pLinkPool);
   }
...
#endif // commented-out exponentially slow graph-splitting algorithm.
...
{
   char *sPronunciation;  /* Combined pronunciation */
   DGraphPtr pDGraph;
   ...
   sPronunciation = SymbolsToString(pSymbols, pLinkPool);
...
static void RemoveDuplicates(char **psPronunList, SDWORD *piNumPronun, char *sKind, char *sPronString, char *sWord)
{
   ...
   for (iOldIndex=iNewIndex=0; iOldIndex<*piNumPronun; iNewIndex++)
   {
      ...
      {
         printf("Duplicate pronu. (%s) in %s list of word '%s'. %s\n",psPronunList[iNewIndex],sKind,sWord,
...
int main(int argc, char *argv[])
{
   ...
   {
      printf("%s                 \r", pVocab->ppWords[iWord]->name);
      ...
      else
      {
         ...
         else printf("%s                 \r", pVocab->ppWords[iWord]->name);
   ...
   if (argc>=4)
   {
      ...
      fp2 = fopen(argv[2],"rt");
      fp3 = fopen(argv[3],"wt");
      while (fgets(sBuf1,10000,fp1) && fgets(sBuf2,10000,fp2))
      {
      ...
      fclose(fp2);
      fclose(fp1);
      printf("%d utterances.  %d shortest in %s; %d shortest in %s; %d same.\n", iShortest1+iShortest2,
