 ...
 *
 */
PHONE * l_sp_ad1_dosyllable(PLTS_T pLts_t, PHONE * wstart, PHONE * endp, int isfinal)
{
 ...
 *
 *      Comments:
			Rules regarding s and syllable division: linguistic rules indicate that when
			the letter s is in a group of two consonants (whether identical or different,
			st, sp, etc.) the consonants are divided between the preceding and the
			following vowel (es-te, a-tis-bar, es-cue-la).
 *
 */
int l_sp_ad1_sonset(PHONE *lsp)
{
	...
	if (lsp == NULL || NEXT(lsp) == NULL)
		return (FALSE);

	if( lsp->p_phone == SP_S )
	{
		if( isvowel(NEXT(lsp)->p_phone) )					// ex. es-te, res-pi-ro
			return TRUE;
		else if( PREV(lsp) && PREV(lsp)->p_phone == 0 )		// ex. staccato -> sta-ka-to
			return TRUE;
	}
