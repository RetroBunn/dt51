...
static char *ls_homo_get_next_word(LPTTS_HANDLE_T phTTS)
{
	PLTS_T pLts_t=phTTS->pLTSThreadData;
	char *temps;

	temps=&(pLts_t->input_array[pLts_t->cur_read_pos]);
	while (temps && *temps && *temps<=' ')
	{
		temps++;
	}
	return(temps);
...
static char *ls_homo_get_prev_word(LPTTS_HANDLE_T phTTS)
{
	PLTS_T pLts_t=phTTS->pLTSThreadData;
	char *temps;
	...
	temps=&(pLts_t->input_array[pLts_t->cur_read_pos]);
	if (*temps>' ') temps--;
	while (temps && *temps && *temps==' ')//use is_white
	{
		temps--;
	}
	while (temps && *temps && *temps>' ')
 ...
 *
 */
struct dic_entry far *ls_homo_homo_464(LPTTS_HANDLE_T phTTS,long index)
{
	...
	if (index==0xDEADD1CF)
	{
		entry_primary=pLts_t->homo_primary;
		entry_secondary=pLts_t->homo_secondary;
	}
	else
	{
	/*
	*  homograph is either right before this or right after ...
	...
	} // else from if (index==0xDAECD1CF)

#ifdef ENGLISH
	/* hack rules for casio */
//	ret = FNX_STRICMP("live",entry_primary->text);
	ret = FNX_STRICMP("l'Iv", &entry_primary->text[1]);
	if (ret==0)
	{	
		temps=ls_homo_get_next_word(phTTS);
		if (FNX_STRNICMP(temps,"from",4)==0 && *(temps+4)==' ')
		{
	...
	//ret = FNX_STRICMP("read", entry_primary->text[1]);
	ret = FNX_STRICMP("r'id", &entry_primary->text[1]);
	if (ret==0)
	{
		temps=ls_homo_get_prev_word(phTTS);
		if (FNX_STRNICMP(temps,"be",2)==0 && *(temps+2)==' ')
		{
			pKsd_t->pronflag=PRON_DIC_ED;
#ifdef PRINTFDEBUG
			if ((DT_DBG(LTS_DBG,0x020)))
				printf("HOMO:CW2\n");
#endif
		}
		if (FNX_STRNICMP(temps,"he",2)==0 && *(temps+2)==' ')
		{
			pKsd_t->pronflag=PRON_DIC_ED;
#ifdef PRINTFDEBUG
			if ((DT_DBG(LTS_DBG,0x020)))
		...
		if (FNX_STRNICMP(temps,"she",3)==0 && *(temps+3)==' ')
		{
			pKsd_t->pronflag=PRON_DIC_ED;
#ifdef PRINTFDEBUG
			if ((DT_DBG(LTS_DBG,0x020)))
		...
		if (pLts_t->fc_struct[pLts_t->fc_index-1]&FC_NAME)
		{
			pKsd_t->pronflag=PRON_DIC_ED;
#ifdef PRINTFDEBUG
			if ((DT_DBG(LTS_DBG,0x020)))
	 ...
	 */
		...
		if(pKsd_t->pronflag & PRON_DIC_ED)
        {
			//pKsd_t->pronflag &= (~PRON_DIC_INTERJECTION);
			if(DICT_FC_ACCESS((*entry_primary).fc[0]) & FC_ED)
			{
#ifdef NEW_LTS
				pLts_t->word_info[pLts_t->cur_word_index].homograph|=HOMO_PRIMARY;
#endif
			...
			if(DICT_FC_ACCESS((*entry_secondary).fc[0]) & FC_ED)
			{
#ifdef NEW_LTS
				pLts_t->word_info[pLts_t->cur_word_index].homograph|=HOMO_SECONDARY;
#endif
	...
	*/
	...
	//This crazy code I believe defaults to a noun when punting...
		// eab 
...
#else
			if(homo_table[i].h_context == 0 ||
			  ((pLts_t->fc_index>1) && (pLts_t->fc_struct[pLts_t->fc_index+1]) && 
			   (homo_table[i].h_context & pLts_t->fc_struct[pLts_t->fc_index-1]) &&
			   (homo_table[i].h_context_after & pLts_t->fc_struct[pLts_t->fc_index+1])) ||
			  ((pLts_t->fc_index>1) && (homo_table[i].h_context_after==0) && 
			   (homo_table[i].h_context & pLts_t->fc_struct[pLts_t->fc_index-1])) ||
			  ((pLts_t->fc_index >= 3) &&
			   (FC_ADV & pLts_t->fc_struct[pLts_t->fc_index-1]) && 
			   (((pLts_t->fc_struct[pLts_t->fc_index+1]) && 
			     (homo_table[i].h_context & pLts_t->fc_struct[pLts_t->fc_index-1]) &&
			     (homo_table[i].h_context_after & pLts_t->fc_struct[pLts_t->fc_index+1])) ||
			   ((pLts_t->fc_index>1) && (homo_table[i].h_context_after==0) && 
			   (homo_table[i].h_context & pLts_t->fc_struct[pLts_t->fc_index-1])))
...
/*					if( (homo_table[i].h_select & DICT_FC_ACCESS((*entry_primary).fc)) == 0)
	...
	if((i < MAX_HOMO_RULE) && homo_table[i].h_suffix)
	{
        ...
        if(pLts_t->fc_index == 1 && pLts_t->length>1)
        {
			if (pLts_t->first_pass==0)
			{
				int i;
		...
		if (printit==0 && DT_DBG(LTS_DBG,0x020))
		{
#ifndef ARM7_NOSWI
#ifndef MSDOS
			if (pKsd_t->dbglog)			/*mfg 04/28/98 added debug support*/
				fprintf((FILE *)pKsd_t->dbglog,"HOMO:(Primary default)\n",i);
#endif 
#ifdef PRINTFDEBUG
			printf("HOMO:(Primary default)\n",i);
#endif
#endif
					}
	return(entry_primary);
...
//#endif //defined (FRENCH || GERMAN || ITALIAN || SPANISH) && defined CASIO_SH3
#endif //#if defined CASIO_SH3 && defined _BIGENDIAN_
...
#ifndef CASIO_SH3
	ret = FNX_STRICMP("read", entry_primary->text);
#else
	ret = FNX_STRICMP("r'id", &entry_primary->text[1]);
#endif
	if (ret==0)
...
#ifndef MSDOS
			if (pKsd_t->dbglog)			/*mfg 04/28/98 added debug support*/
				FNX_FPRINTF((FILE *)pKsd_t->dbglog,"HOMO:(Primary default)\n",i);
#endif 
#ifdef PRINTFDEBUG
			FNX_PRINTF("HOMO:(Primary default)\n",i);
#endif
#endif
					}
	return(entry_primary);
