...
//	}
	...
	*/
	...
	/* Use the correct KS entry based on language being compiled */
#ifdef USE_NAND
	if(DICT_ENTRY == 0 && (pKsd_t->fdicFileHandle ==0))
#else
	if(DICT_ENTRY == 0)
#endif
	{
...
int ls_dict_find_word(LPTTS_HANDLE_T phTTS, int nosend)
{
	...
	int stat=MISS;	/* what the lookup returned */
	...
	do
	{
		...
		if (pKsd_t->halting)	//added to exit early if reset called
			return(HIT);
...
file_system_lookup:
	...
	if (stat != HIT)
#ifdef USE_NAND
	/* do the lookup in the file system dictionary here */
	/* call ls_dict_dlook with a special value (0xDEADD1CF) to tell it 
	   to use the file system dictiaonry.  */
	{
		stat=ls_dict_file_system_lookup(phTTS,&localoff,&FileEnt);
		if (stat!=HIT)
	...
	if (FileSystemHit)
	{
		if(pLts_t->fc_struct[pLts_t->fc_index] == 0)
			pLts_t->fc_struct[pLts_t->fc_index] = (pent->fc[0]);
		else 
		{
			if((pent->fc[0]) & FC_HOMOGRAPH)
			{
...
//		}
	...
	{
#ifdef USE_NAND
        if( (FileSystemHit && (((pent->fc[0]) & PPHRASE)== PPHRASE)) ||
			(DICT_FC_ACCESS(pent->fc[0]) & PPHRASE) == PPHRASE) 
#else
	...
	{
		if (nosend==0)
			ls_util_send_asky_phone_list(phTTS,pent->text+localoff+1);
	}
	else
	{
...
//#endif
	...
	{             
		...
		{
#if defined CASIO_SH3 && (defined (FRENCH) || defined (GERMAN) || defined (ITALIAN) || defined (SPANISH))
			*ppent = ls_homo_homo_464(phTTS,index);
#else
...
#ifdef USE_NAND
int ls_dict_file_system_lookup(LPTTS_HANDLE_T phTTS, int *pLocaloff, struct dic_entry  *pent)
{
	struct dic_entry dic_entry1,dic_entry2;

	PKSD_T pKsd_t=phTTS->pKernelShareData;
	PLTS_T pLts_t=phTTS->pLTSThreadData;
//#ifdef ENGLISH
	struct dic_entry *ppent;
//#endif
	int res=0;
	int length=0;
	dic_entry1.text[0]='\0';
	...
	if (pKsd_t->fdicFileHandle ==0)
		return(MISS);

	pLts_t->hit_type=HIT;

	if (pLts_t->abbrev_look)
	{
		length=strlen(pLts_t->comp_str);
		pLts_t->comp_str[length]='.';
		pLts_t->comp_str[length+1]='\0';
		res=FindWord(pLts_t->comp_str,&(dic_entry1.text[1]),&(dic_entry1.fc[0]),
			         &(dic_entry2.text[1]),&(dic_entry2.fc[0]),
				     pKsd_t->fdicFileHandle);
		pLts_t->comp_str[length]='\0';
		if (res==HIT)
		{
			pLts_t->hit_type=ABBREV;
		}
	...
	/* check for homograph.. */
	if(pKsd_t->lang_curr==LANG_english && dic_entry1.fc[0] & FC_HOMOGRAPH)
	{
		if( dic_entry1.fc[0] & (FC_HOMOGRAPH | FC_CHARACTER) )
		{
			ppent=&dic_entry1;
			pLts_t->homo_primary=&dic_entry1;
			pLts_t->homo_secondary=&dic_entry2;
		}
		else
		{
			ppent=&dic_entry2;
			pLts_t->homo_primary=&dic_entry2;
			pLts_t->homo_secondary=&dic_entry1;
		}
#if defined CASIO_SH3 && (defined (FRENCH) || defined (GERMAN) || defined (ITALIAN) || defined (SPANISH))
		ppent = ls_homo_homo_464(phTTS,0xDEADD1CF);
#else
		ppent = ls_homo_homo(phTTS,0xDEADD1CF);
#endif
		pent->fc[0]=ppent->fc[0];
		pent->text[0]='\0';
		strcpy(&(pent->text[1]),&(ppent->text[1]));
	}
	else
	{
		pent->fc[0]=dic_entry1.fc[0];
		pent->text[0]='\0';
		strcpy(&(pent->text[1]),&(dic_entry1.text[1]));
	}
	*pLocaloff=0;
	return(HIT);
}
...
MMRESULT AddUserEntry(LPTTS_HANDLE_T phTTS, struct dic_entry *entry)
{
	...
	if (UDICT_INDEX == NULL)
	{
		...
		if (!(UDICT_INDEX_LVALUE = (S32 *) mallocLock(4)))
#else
		if (!(UDICT_INDEX_LVALUE = (S32 *) FNX_MALLOC(4)))
#endif
	...
	if ((UDICT_INDEX_LVALUE = reallocLock(UDICT_INDEX, (UDICT_ENTRY+1) * sizeof(S32))) == NULL)
	{
		return MMSYSERR_NOMEM;
	}
...
#else
	if ((UDICT_INDEX_LVALUE = FNX_REALLOC(UDICT_INDEX, (UDICT_ENTRY+1) * sizeof(S32))) == NULL)
	{
		return MMSYSERR_NOMEM;
	}
...
//	bytes_to_scoot =  UDICT_BYTES - (UDICT_INDEX[index+1]); 
	...
	UDICT_INDEX_LVALUE = reallocLock(UDICT_INDEX, (UDICT_ENTRY-1) * sizeof(S32));
	UDICT_DATA = reallocLock(UDICT_DATA, new_size);
#else
	UDICT_INDEX_LVALUE = FNX_REALLOC(UDICT_INDEX, (UDICT_ENTRY-1) * sizeof(S32));
