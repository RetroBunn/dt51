...
void ls_proc_do_digit_group(LPTTS_HANDLE_T phTTS, unsigned char buf[3], int oflag, int cflag)
{
  FLAG Fin=FALSE;

  if (buf[0] != '0') {
    if (buf[0]!='1') {	/* 200, 257; pas 020, 100 */
		ls_util_send_phone_list  (phTTS, punitsh [buf[0]-'0']);
      ls_util_send_phone (phTTS, WBOUND);
    }
    ls_util_send_phone_list (phTTS, phundred);
  ...
  if (!Fin) {
	...
	ls_util_send_phone_list (phTTS, dizieme);    /* 10ème */
		  ...
		  if (cflag)
		  {
			  	ls_util_send_phone_list (phTTS, pteensh [buf[2]-'0']);
		  } else {
  ...
  if (!Fin) {
    if (oflag) {
      ls_util_send_phone_list (phTTS, pordin [buf[2]-'0']);
		...
		if (cflag){
			ls_util_send_phone_list (phTTS, punitsh [buf[2]-'0']);
		} else {
		...
		else 
		{
			if (ndig != 0) 
			{
				...
				{	/* Thousands		*/
					...
					{
						ls_proc_do_digit_group(phTTS,&buf[12], FALSE, TRUE);
						ls_util_send_phone(phTTS,WBOUND);
					}
					ls_util_send_phone_list(phTTS,pthousand);
				...
				ls_task_lookahead_readword (phTTS, pLts_t->nword);
				temp=(char)((pLts_t->nword[0]).l_ch);
				if ((lsctype[temp]& C) && temp!='h' && temp!='H')
				{
				ls_proc_do_digit_group(phTTS,&buf[15], oflag,TRUE);	/* Units		*/
				}
				else
				{
