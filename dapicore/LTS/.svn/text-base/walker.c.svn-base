...
// REWT
// temporary change for investigation: 
// Write all words that can't be parsed by
// word grammar to a file. Uncomment the following to activate.
// #define REMEMBERFAILEDPARSES

#ifdef REMEMBERFAILEDPARSES
static FILE *logfile;
static int  logfileisopen;
...
int long_or_short(ASCIILETTER cin, ASCIILETTER cr, ASCIILETTER crr, SHORT *explain)
{
  
  const  ASCIILETTER *shortlist[] = {"x","ß", "sch", NULL};         
  const ASCIILETTER *longlist[] = {"sm", "sh","ph","rf","rl","rk","rg","rh","dr", NULL};
  int lang, len;
  int i;
...
int  match_whatever(PLTS_T pLts_t, ASCIILETTER *word, 
...
void tolowerGER(ASCIILETTER *word, ASCIILETTER *outword)
	...
	in which case the content of the input gets overwritten */
{
  ...
  for (p=word,pp=outword; (c = *p) != 0; p++)
	...
	default:
	  if (c>='A'&& c<='Z') cc = (ASCIILETTER) (c+32);
	  else  cc = c;
	  break;
	}
      *pp++ = cc;
...
void toupperGER(ASCIILETTER *word, ASCIILETTER *outword)
...
void register_word_part(WalkCNTXT *cntxt, ASCIILETTER *name, U16 from, U16 to, BOOL action, ASCIILETTER *phone, U16 rule)
...
#ifdef FROMTYPED     
void display_word_stucture(ASCIILETTER *word, MorphemeStruct *M)
{
  int i;
  FNX_PRINTF("%s @@ \n",word);
...
void process_stress(ASCIILETTER *phonemes)
{
  ...
  fleftwind = 7;              // stresses before primary stress less than fleftwind are suppressed.
  frightwind = 3;              // stresses after primary stress less than frigthwind are suppressed.
  ...
  if(focus)
	...
	{
	  fleftwind = 7;              // stresses before primary stress less than fleftwind are suppressed.
	  frightwind =4;              // stresses after primary stress less than frigthwind are suppressed.
...
BOOL is_shortener(ASCIILETTER c1, ASCIILETTER c2)
{

  /* these consonant combinations, enclosed by vowels shorten the preceeding vowel */

  const  ASCIILETTER *shortenerlist[] = {"sp", "st", "rd", "rl", "rt", 
				"ld", "lt", "ng", "tz", "nz", 
				"mp", "nt", "nd", "pf", "lk",
                                "rg", "kt"}; 
  int i;
...
BOOL morphemize_word(void * pLts_t,const ASCIILETTER *word, 
		     ASCIILETTER *outword, 
		     ASCIILETTER *outphonemes, 
		     WalkCNTXT *Contxt,
...
#ifdef REMEMBERFAILEDPARSES
	  if (logfileisopen != 999 )
	  {
		  logfile = fopen("germanlogfile.txt","a");
		  logfileisopen = 999;
	  }
	  if (logfileisopen == 999 && logfile)
	  {
		  fprintf(logfile,"%s \n",  wordbuff);
	  }
#endif
