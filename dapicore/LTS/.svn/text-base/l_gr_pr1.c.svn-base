	...
	if (buf[1] != '0') 
	{
		if (buf[2] != '0')
		{
			if (oflag != FALSE)
			{
				if (buf[1] == '0')
				{
					if (pLts_t->german_number_gender==E_WORD)
					{
						ls_util_send_phone_list(phTTS,pordine[buf[2]-'0']);
					}
					else if (pLts_t->german_number_gender==EN_WORD)
					{
						ls_util_send_phone_list(phTTS,pordinen[buf[2]-'0']);
					}
					else // ER_WORD
					{
			...
			if (ndig != 0) 
			{
				...
				if (oflag != FALSE)
				{
					if (buf[16] >= '2' ||  (buf[15] != '0' && buf[16] == '0' && buf[17] == '0'))
					{
						...
						{
						   ls_util_send_phone_list(phTTS, pstexn);
						}
						else // ER_WORD
						{
					   ls_util_send_phone_list(phTTS, pstexr);
					...
					else
					{
					   if (buf[16] != '0')
					   {
						...
						{
						   ls_util_send_phone_list(phTTS, ptexn);
						}
						else // ER_WORD
						{
	...
	if (lp1 != rlp) {
		...
		if (lp1+3 == rlp) 
		{
			ls_proc_do_2_digits(phTTS,lp1+1,TRUE);
				...
				if (((lp1+1)->l_ch != '1') || (((lp1+1)->l_ch == '1') & ((lp1+2)->l_ch == '0')))
					pLts_t->pflag = ls_proc_do_number(phTTS,lp1, rlp, ls_util_is_dot(pLts_t));
				else
					ls_proc_do_4_digits(phTTS,lp1+1);
