...
extern void register_word_part(WalkCNTXT *cntxt, ASCIILETTER *name, U16 from, U16 to, BOOL action, ASCIILETTER *phone, U16 rule);
...
/*
  ...
  ASCIILETTER *BLUB.phonemestring = NULL;
...
newalt:	    
	...
	*/
	...
	cntxt->STK[NST].nalt = Lt_parse_string(cntxt->theword,
			       ...
			       (U16)((cntxt->Nodes[cntxt->STK[NST].curr].info.parameter) & MORPH_ORDER_MASK));  // parameter: LONG MORPH first or last
	cntxt->STK[NST].next = cntxt->Nodes[cntxt->STK[NST].curr].right;
		...
		if (cntxt->STK[NST].newwalki.succeeded)   // get the phoneme string info, and rule number i.
		  {
		    ...
		    cntxt->STK[NST].phonemestring =  (ASCIILETTER *)point_to_phoneme_string(cntxt->MorphDict, cntxt->STK[NST].phonemeindex[cntxt->STK[NST].inx]);  //(ltrie.c)

		    register_word_part(cntxt,cntxt->Nodes[cntxt->STK[NST].curr].name,
				       cntxt->STK[NST].currpos,(U16)(cntxt->STK[NST].currpos+cntxt->STK[NST].proceed[cntxt->STK[NST].inx]),0,
