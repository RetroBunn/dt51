...
#ifdef GERMAN
extern int ls_task_set_german_number_gender(LPTTS_HANDLE_T phTTS);
#endif
#ifdef JAPANESE
extern int ls_rule_do_japanese(LPTTS_HANDLE_T phTTS, char *sText, BOOL bSpeak);
#endif
#ifdef KOREAN
extern void ls_rule_korean_lts(LPTTS_HANDLE_T phTTS);
#endif
#ifdef SPANISH
extern void ls_rule_spanish_lts(LPTTS_HANDLE_T phTTS);
#endif
#ifdef LATIN_AMERICAN
extern void ls_rule_laspanish_lts(LPTTS_HANDLE_T phTTS);
#endif
...
#elif (!defined WIN32_STATIC && !defined EPSON_ARM7) || defined MULTIPLE_LANGUAGES_LOADED
...
//	int i;
	...
	if ((input[0]&PFONT)==(PFASCII<<PSFONT))
	{
		...
		if (input[0]==0x0b) //MGS why is it doing this? is there a reason.  it will probably break say letter mode
		{
			pLts_t->input_array[pLts_t->cur_input_pos++]=(' ');
		}
		
		pLts_t->input_array[pLts_t->cur_input_pos++]=(input[0]&PVALUE);
		pLts_t->input_array[pLts_t->cur_input_pos] = 0x00;		// Null terminate
		...
		if (pKsd_t->halting)
			...
			if (((phTTS->ph_funcs.char_types[pLts_t->input_array[temp]])&MARK_space) && 
			  (pLts_t->input_array[temp-1]=='\'') && 
			  ((temp>=2) && (FNX_TOLOWER(pLts_t->input_array[temp-2])=='s')) ) 
			{
				return;
			}
			...
			pLts_t->cur_index=-1;
			memset(&(pLts_t->pword),0,sizeof(pLts_t->pword));
		}
		else
		{
			return;
		...
		switch (input[0]&(PFONT|PVALUE))
		{
			...
			case INDEX_REPLACE_END:	
				...
				if( phTTS->cur_lts_lang==LANG_japanese || 
					phTTS->cur_lts_lang==LANG_chinese ||
...
void lts_main_loop(LPTTS_HANDLE_T phTTS)
{
	...
	ls_rule_do_japanese(phTTS, NULL, TRUE);
	return;
#elif defined(CHINESE)
	extern int Chinese_ProcessPinyin(LPTTS_HANDLE_T phTTS, CHAR *inword);
	pLts_t->cur_read_pos=0;
	Chinese_ProcessPinyin(phTTS, pLts_t->input_array);
	return;
#elif defined(KOREAN)
	pLts_t->cur_read_pos=0;
		 ...
		 second_time==0)
	{
...
#ifdef GERMAN
		if (!done)
			done=ls_task_set_german_number_gender(phTTS);
#endif
		...
		// copy word to previous word storage
		memcpy(&(pLts_t->pword),&(pLts_t->cword),sizeof(pLts_t->cword));
...
//#endif				
		...
		case '?':
			if ((pKsd_t->modeflag & MODE_INTONREV))
			{
				// hack for casio to reverse the intonation of the questions 
				if (pLts_t->wstate==IS_WH)
				{
					pLts_t->wstate= NOT_WH;
				}
				else if (pLts_t->wstate==NOT_WH)
				{
					pLts_t->wstate =IS_WH;
				}
			}
...
/* MGS 8/20/97 hack for stressing first words in the sentence for kerzweil */
	  	...
	  	if ((ls_task_wlookup(pLts_t, llp, (unsigned char *)&spwhword[0])) != NULL)
		{
			...
			*/
		   ls_util_send_phone(phTTS,SEMPH);
			pLts_t->wstate = SP_WH;
		//	goto out; 
		}
...
#ifdef ITALIAN
		/*igrega or i code*/
		if ((c==(unsigned char)0xe8 || c==(unsigned char)0xc8)) //0xe8 = 'è' //0xc8 = 'È'
		{
#if 0
		 if(((pLts_t->lflag)&(LSTRIP|RSTRIP))==0)
			 //&& ls_util_is_white(&(pLts_t->citem))!=FALSE)
		{
			ls_util_send_phone(phTTS,S1);
		 }
		 else
		 {
			...
			pLts_t->fc_struct[pLts_t->fc_index] = FC_CONJ;
			ls_util_send_phone(phTTS,IT_EH);
#ifdef LS1DEBUG
			FNX_PRINTF(" iggre to i  code \n");
#endif
#endif 
...
//				ls_util_send_phone(phTTS,S1);
				...
				{
#ifdef ITALIAN
					ls_util_send_phone_list(phTTS,peuros);					
#else
	...
	if (lp1==rlp && Trouve_Voy) { /* analyse the entire word */
		...
		*/
		...
		{  
			...
			*/ 
			ls_task_lookahead_readword (phTTS, pLts_t->nword);     /* "les" */

			ls_rule_lts (phTTS, llp, rlp, TRUE); /* "tous" */
...
//#ifndef ARM7
	...
	low=1;
	high=sizeof(verb_pairs_index)/sizeof(verb_pairs_index[0]) - 1;
...
int ls_task_search_for_phrasal_verbs(PLTS_T pLts_t)
{
#if defined ENGLISH_US
	int i,j;
	int num_pverbs=sizeof(verb_pairs_cont2)/(sizeof(char)*2);
	unsigned char value;
	unsigned char temp;
	int found_it=0;
	...
	for (i=0;i<pLts_t->fc_index;i++)
	{
		if (pLts_t->pro_markers[i] & 0xFF000000)
		{
			...
			for (j=0;j<num_pverbs && value>=verb_pairs_cont2[j][0] && found_it==0;j++)
			{
				if (verb_pairs_cont2[j][0]==value)
				{
					if ((i+1)<pLts_t->fc_index)
					{
						temp=(unsigned char)((pLts_t->pro_markers[i+1] & 0xFF000000)>>24);
						if (temp==verb_pairs_cont2[j][1])
						{
							found_it=1;
						}
					}
			...
			if (found_it)
			{
				pLts_t->pro_markers[i]|=PRO_VERB_PHRASE;
				i+=found_it;
			}
		}
	}
...
void ls_task_parse_sentence(LPTTS_HANDLE_T phTTS)
{
	...
	pLts_t->fc_struct[0] = 0;
	pLts_t->fc_struct[1] = 0;
	pLts_t->fc_struct[2] = 0;
	pLts_t->pro_markers[0]=0;
	pLts_t->pro_markers[1]=0;
	pLts_t->pro_markers[2]=0;
	for (i=0;i<pLts_t->cur_input_pos;i++)
	{
		for (pLts_t->cur_read_pos=i;(pLts_t->cur_read_pos<pLts_t->cur_input_pos && 
			((phTTS->ph_funcs.char_types[pLts_t->input_array[pLts_t->cur_read_pos]])&MARK_space) ); pLts_t->cur_read_pos++,i++);
			
		if (i>=pLts_t->cur_input_pos)
			break;

		k=0;
		for (pLts_t->cur_read_pos=i;(pLts_t->cur_read_pos<pLts_t->cur_input_pos && 
			(!((phTTS->ph_funcs.parser_char_types[pLts_t->input_array[pLts_t->cur_read_pos]])& (TYPE_white  | TYPE_clause))) && k<199);
				pLts_t->cur_read_pos++,k++,i++)
		{
			cur_word[k].l_ch=pLts_t->input_array[pLts_t->cur_read_pos];
		}
		start=0;
		cur_word[k].l_ch=0;
#if defined ENGLISH_US || defined ITALIAN
...
//			pLts_t->pro_markers[pLts_t->fc_index]|=PRO_PREP;
		...
		if((pLts_t->fc_struct[pLts_t->fc_index]&FC_TO))
		{
			//pLts_t->pro_markers[pLts_t->fc_index]|=PRO_TO;
		}
		...
		pLts_t->fc_struct[pLts_t->fc_index+1]=0;
		pLts_t->pro_markers[pLts_t->fc_index+1]=0;
		pLts_t->fc_struct[pLts_t->fc_index+2]=0;
		pLts_t->pro_markers[pLts_t->fc_index+2]=0;
	...
	{	
		if (((pCmd_t->skip_mode & SKIP_conj)==0)) 
		{
			
#if !defined ARM7 || defined ITALIAN
...
//					}
					...
					if (i-last_marker>=4)
					{
						...
						{
							pLts_t->pro_markers[i] &= (~0x000FFFFF);
							continue;
						}
...
#endif
					if (i-last_marker>=2 && (find_next_marker(pLts_t,i)>=2) 
						&& (pLts_t->pro_markers[i] & (PRO_TO | PRO_THAT)))
					{
						pLts_t->pro_markers[i]|=PRO_PREP_BREAK;
						last_marker=i;
						continue;
					}
					if (find_next_marker(pLts_t,i)>=2)
...
//						pLts_t->pro_markers[i]|=PRO_OPT_BREAK;
...
#if defined ENGLISH_US
		ls_task_search_for_phrasal_verbs(pLts_t);
#endif
		...
		if (DT_DBG(VTM_DBG,0x002))
		{
			...
			for (j=0;j<pLts_t->cur_input_pos;j++)
			{
				...
				{
					FNX_PRINTF("      ");
					if (pLts_t->pro_markers[i]& PRO_REQ_BREAK)
					{
						FNX_PRINTF("PRB ");
					}
					if (pLts_t->pro_markers[i]& PRO_OPT_BREAK)
					{
						FNX_PRINTF("POB ");
					}
					if (pLts_t->pro_markers[i]& PRO_VERB_PHRASE)
					{
						FNX_PRINTF("PVB ");
					}
					if (pLts_t->pro_markers[i]& PRO_PREP_BREAK)
					{
						FNX_PRINTF("PPB ");
					}
...
//	pLts_t->fc_struct[0] = 0;
//	pLts_t->fc_struct[1] = 0;
#endif //	#if defined ENGLISH_US || defined ITALIAN

}

#ifdef GERMAN

int ls_task_set_german_number_gender(LPTTS_HANDLE_T phTTS)
{
	PLTS_T pLts_t=phTTS->pLTSThreadData;
	char *cp;

	cp = ls_task_wlookup(pLts_t, &(pLts_t->pword[0]), (unsigned char *)&e_ending_words[0]);
	if (cp!=NULL)
	{
		pLts_t->german_number_gender=E_WORD;
		return(0);
	}
	cp = ls_task_wlookup(pLts_t, &(pLts_t->pword[0]), (unsigned char *)&en_ending_words[0]);
	if (cp!=NULL)
	{
		pLts_t->german_number_gender=EN_WORD;
		return(0);
	}
	
	pLts_t->german_number_gender=ER_WORD;
	return(0);
}
#endif
