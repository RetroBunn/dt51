...
/* GL 04/21/1997  add this for OSF build */
		 ...
		 FNX_STRICMP(FNX_STRRCHR(pCmd_t->pString[0],'.'),".au")==0)
	{
		is_au=1;
		/* open the file */
		if ((au_fd=_open(pCmd_t->pString[0],O_RDONLY))==-1)
		{
		...
		if (_read(au_fd,&AuFileHdr,sizeof(AuFileHdr)-4)<sizeof(AuFileHdr)-4)
		{
			TextToSpeechErrorHandler( phTTS,
		...
		if (_lseek(au_fd,AuFileHdr.hdr_size,SEEK_SET)<0)
		{
			TextToSpeechErrorHandler( phTTS,
				  ERROR_BAD_WAVE_FILE_FORMAT,
				  MMSYSERR_ERROR );
...
/* GL 04/21/1997  add this for OSF build */
	...
	if (is_au)
	{
		iLength = _read(au_fd,lpData,uiReadSize);
	}
	else
	{
