 ...
 * 0024 GL		03/05/1997		Move r_modulation_464() from l_gr_dr1.c to here.
 * 0024 GL		03/05/1997		Move r_modulation_464() from l_gr_dr1.c to here.
...
const short lplos_build_time_464=7;
const short bplos_build_time_464=7;
//short tmp;
...
#include "dbgwins.h"
#endif
void r_modulation_464 (LPTTS_HANDLE_T phTTS,short *formpointer, short percent, short temp);
void rit_modulation_464 (LPTTS_HANDLE_T phTTS,short *formpointer, short percent, short temp);
void rs_modulation_464 (LPTTS_HANDLE_T phTTS,short *formpointer, short percent, short temp);
void gr_modulation_464 (LPTTS_HANDLE_T phTTS,short *formpointer, short percent, short temp);
void h_modulation_464 (LPTTS_HANDLE_T phTTS,short *formpointer, short percent, short temp);
//static void add_feature (PDPH_T pDph_t
void Phrase_intial_sil_start_464(LPTTS_HANDLE_T phTTS);
void Phrase_final_silence_464(LPTTS_HANDLE_T phTTS);
void	Set_pressure_464(LPTTS_HANDLE_T phTTS);
void Do_ue_dc_464(LPTTS_HANDLE_T phTTS);
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/* D R A W P A R S                                  					 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*
 ...
 */
	...
	const short nasalization_464[13]={0, 30, 60, 100, 130, 160, 190,320,450,570,675,780,885};
	//const short nasalization[13]={0, 150, 200, 300, 400, 500, 600,700,800,800,805,8100,885};
	const short velar_f1_464[8] = { 180,220,260,280,320,390,400,410};
	const short dcval_464[11] = { 0, 30,40,80,90, 95,96, 100, 110,105,120};

	const  short ueval_464[11] = { 0,40,60,80,90,100,115,120,125,125,130};


void phdraw_464 (LPTTS_HANDLE_T phTTS)
{
...
if(pKsd_t->lang_curr == LANG_italian)
{
	for (; np <= &PTONGUEBODY; np++)
	{
		...
		if(np == &PAREAL)
		{
			if(np->tspesh)
			{
				...
				else if ( pDph_t->tcum  >= (np->tspesh-(bplos_build_time_464) ) 
		...
		if(np == &PAREAB)
		{
			if(np->tspesh)
			{
				...
				else if (pDph_t->tcum  >= (np->tspesh-  (bplos_build_time_464+1)) 
					&& (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FBURST))
				{
	...
	{	
		Phrase_intial_sil_start_464(phTTS);
#ifdef HLSYN
		Do_ue_dc_464(phTTS);
#endif
	}
				...
				{
		Phrase_final_silence_464(phTTS);
			}		
...
//end of new tb stuff
		...
		if(pDph_t->in_tbrelease || (pDph_t->tstep && pDph_t->tstep <7))
		{
			...
			if(pDph_t->tstep <= 3 )
			{
				...
				{
					pDph_t->parstochip[OUT_F1]= velar_f1_464[pDph_t->tstep];
			...
			{
				if (velar_f1_464[pDph_t->tstep] < 300 )
					pDph_t->parstochip[OUT_F1] = velar_f1_464[pDph_t->tstep];

				else if(pDph_t->parstochip[OUT_F1] < velar_f1_464[pDph_t->tstep])
				{
				pDph_t->parstochip[OUT_F1] = ((pDph_t->parstochip[OUT_F1] + velar_f1_464[pDph_t->tstep]) >>1);
				}
				if(pDph_t->tstep >= 7)
...
//		pDph_t->tbstep);
	...
	if ( pDph_t->allophons[ pDph_t->nphone] == SPP_J)
	{
		...
		r_modulation_464 (phTTS,pDph_t->param[A2].outp, N10PRCNT, TRUE);
	}
	if (pDph_t->allophons[ pDph_t->nphone] == GRP_KH )
	{
		r_modulation_464 (phTTS,pDph_t->param[A2].outp, N10PRCNT, FALSE);
		r_modulation_464 (phTTS,pDph_t->param[A3].outp, N20PRCNT, FALSE);
		r_modulation_464 (phTTS,pDph_t->param[AV].outp, N8PRCNT, TRUE);
	}
		...
		// close tongue body and then modulate my zero mass tongue body 
				...
				//hack for 5.0 release fix it prettier later
			{
				...
				if (phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone-1)]) & FVOWEL )
				{
					...
					gr_modulation_464 (phTTS,&pDph_t->parstochip[OUT_ATB], N40PRCNT, TRUE);
				}
				else
				{
			...
			else
			{
				...
				gr_modulation_464 (phTTS,&pDph_t->parstochip[OUT_ATB], N90PRCNT, TRUE);
				pDph_t->modulcount++; //increase modulation speed
			}
#else
if ((phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone-1)]) & FVOWEL) IS_PLUS)
		{
			r_modulation_464 (phTTS,pDph_t->param[AB].outp, 0, FALSE);
			r_modulation_464 (phTTS,pDph_t->param[AV].outp, N5PRCNT-300, FALSE);
			r_modulation_464 (phTTS,pDph_t->param[A2].outp, N20PRCNT, FALSE);
			r_modulation_464 (phTTS,pDph_t->param[A3].outp, N20PRCNT, TRUE);
		}
		else
		{
			r_modulation_464 (phTTS,pDph_t->param[AB].outp, 0, FALSE);
			r_modulation_464 (phTTS,pDph_t->param[AV].outp, N5PRCNT, FALSE);
			r_modulation_464 (phTTS,pDph_t->param[A2].outp, N10PRCNT, FALSE);
			r_modulation_464 (phTTS,pDph_t->param[A3].outp, N10PRCNT, TRUE);
		}
#endif
 ...
 //		pDph_t->modulcount =0;
	...
	if ( pDph_t->allophons[ pDph_t->nphone] == 8099)
	{
		...
		{	
			...
			r_modulation_464 (phTTS,pDph_t->param[AREAB].outp, N100PRCNT, TRUE);
			
			
#else
			
			
			//	rit_modulation_464 (phTTS,pDph_t->param[A2].outp, N10PRCNT, FALSE);
			//a2 is an index into tables for non_hlsyn don't modulate it
			//	rit_modulation_464 (phTTS,pDph_t->param[A3].outp, N20PRCNT, FALSE);
			//if av isn't one yet due to plosive wait
			if(*pDph_t->param[AV].outp >30)
			r_modulation_464 (phTTS,pDph_t->param[AV].outp, N70PRCNT, TRUE);
		...
		|| pDph_t->allophons[ pDph_t->nphone] == ITP_R)
	{
		...
		{		
			...
			//tr blend is a hack at the moment
			pDph_t->parstochip[OUT_A2] = 2100;
			rs_modulation_464(phTTS,pDph_t->param[AREAB].outp, N100PRCNT, TRUE);
			...
			rs_modulation_464 (phTTS,pDph_t->param[AV].outp, N90PRCNT, FALSE);
			rs_modulation_464 (phTTS,pDph_t->param[F2].outp, N20PRCNT, TRUE);
			}
...
#ifdef HLSYN
		r_modulation_464 (phTTS,pDph_t->param[AREAB].outp, N90PRCNT, TRUE);
#else
		r_modulation_464 (phTTS,pDph_t->param[AV].outp, N20PRCNT, FALSE);
		r_modulation_464 (phTTS,pDph_t->param[A2].outp, N20PRCNT, FALSE);
		r_modulation_464 (phTTS,pDph_t->param[A3].outp, N20PRCNT, TRUE);
 ...
 //for hebrew phonemes
	...
	if(pDph_t->allophons[pDph_t->nphone ] == UKP_NX)
	{
		*pDph_t->param[AREAG].outp = 450;
			r_modulation_464 (phTTS,pDph_t->param[AREAG].outp, N50PRCNT, TRUE);
	}
	...
	if (pDphsettar->phcur == USP_CZ	)
	{
			...
			*pDph_t->param[AREAB].outp =400;
			...
			h_modulation_464 (phTTS,pDph_t->param[AREAB].outp , N90PRCNT, TRUE);
				//	pDph_t->parstochip[OUT_F1] = 270;
			//gr_modulation_464 (phTTS,&pDph_t->parstochip[OUT_F1], N10PRCNT, TRUE);
#else
			*pDph_t->param[AP].outp = 50;
			h_modulation_464 (phTTS,pDph_t->param[AP].outp , N20PRCNT, TRUE);
			*pDph_t->param[AV].outp = 20;
			h_modulation_464 (phTTS,pDph_t->param[AV].outp , N20PRCNT, TRUE);
#endif
...
/*
 *      Function Name: r_modulation_464()     
 *
 ...
 *      Return Value: void
 *
 *      Comments: Seems to be about the same as k_modulation_464; is used in phdraw()
 *					but k_modulation_464 is not (don't know why).
 *
 */
				...
				if(pDph_t->dcstep != 0)
				{
					...
					if(pDph_t->dcstep >= 0)
					{
						...
						if(place(pDph_t->allophons[pDph_t->nphone]) & FLABIAL)
						{
							...
							pDph_t->parstochip[OUT_DC] = dcval_464[pDph_t->dcstep]; 
							pDph_t->parstochip[OUT_UE] = ueval_464[pDph_t->dcstep];
