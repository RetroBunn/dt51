...
void find_syll_to_stress (LPTTS_HANDLE_T phTTS, short *locend, short nstartphrase, short nstresses);
...
void add_feature_j (PDPH_T pDph_t, long feaname, short location);
...
// from p_la_sr1.c
#if (!defined CASIO_SH3 && !defined _BIGENGIAN_)
...
//wbound pp case as it is redundant
			...
			{
				if( pDph_t->symbols[n] ==  pDph_t->symbols[n+1]
					&&  (pDph_t->symbols[n]& PVALUE) <100)
				{
					 pDph_t->symbols[n] = S1;
				}
			}
			...
			//reiner rule to compensate for bachus putting in a glottal stop in front of first word in an uttereance
			...
			if( pDph_t->symbols[n]  == LAP_NH )		// Turn NH into an enye sound
			{
				insertphone(phTTS, (short)(n+1), LAP_I);
				pDph_t->symbols[n]  = LAP_N;		// Ahhh, NH causes the I to shorten too much and I don't know why
				n++;
			}
			if( pDph_t->symbols[n]  == SPP_NH )		// Turn NH into an enye sound
			{			
				insertphone(phTTS, (short)(n+1), SPP_I);
				pDph_t->symbols[n]  = SPP_N;		// Ahhh, NH causes the I to shorten too much and I don't know why
				n++;
			}

			if( pDph_t->symbols[n]  == HEP_AL && n> 2)	//if not at the begining use the Q
				pDph_t->symbols[n] =USP_Q;
...
// the synthesizer can't really do a plost_firc in one phoneme so seperate them.
		...
		{
			pDph_t->symbols[n] = GRP_T;
			insertphone (phTTS, (short)(n+1), GRP_S);
		}
		if(pDph_t->symbols[n] == ITP_TS)
		{
			pDph_t->symbols[n] = ITP_T;
			insertphone (phTTS, (short)(n+1), ITP_S);
		}
	...
	if( pDph_t->symbols[n]  == USP_AX && 
		pDph_t->symbols[n-2]  >= WBOUND && 
		(pDph_t->symbols[n-1]  == S1 || pDph_t->symbols[n-1]  == SEMPH)	&& 
		pDph_t->symbols[n+1]  == WBOUND )
	{
	...
	if (((pDph_t->symbols[n]& PVALUE) >= S2) && ((pDph_t->symbols[n]& PVALUE) <= SEMPH))
	{
		...
		while (((pDph_t->symbols[m] & PVALUE) >= MAX_PHONES) && (m < pDph_t->nsymbtot))
		{
			if( ((pKsd_t->lang_curr == LANG_english || 
				...
				((pDph_t->symbols[m] & PVALUE) > WBOUND && 
				(pDph_t->symbols[m] & PVALUE) < NEW_PARAGRAPH && 
...
stzapped:
	...
	if (((pDph_t->symbols[n] & PVALUE) >= SBOUND) && ((pDph_t->symbols[n] & PVALUE) <= EXCLAIM))
	{
		...
		if(pKsd_t->lang_curr == LANG_english
			...
			|| pKsd_t->lang_curr == LANG_british)
			if (((pDph_t->symbols[n]& PVALUE) >= COMMA) && ((pDph_t->symbols[n]& PVALUE) <= EXCLAIM))
				find_syll_to_stress (phTTS, &n, nstartphrase,nstresses);	/* MVP : was n and n was static */
	...
	//	printf("FOUND a VERB %d\n",pDph_t->wordclass[n] & FC_VERB);
		...
		{	
				...
				pKsd_t->lang_curr == LANG_korean )
			{
				...
				{
					add_feature (pDph_t, FWINITC, (short)(CURRPHONE));
				}
			}
...
do_japanese:
			...
			case PERIOD:
				...
				last_bound = 0;
				add_feature (pDph_t, FSENTENDS, (short)(CURRPHONE));
				make_phone (pDph_t, GEN_SIL, n,&mf0, curr_dur, curr_f0);
				word_init_sw = TRUE;
				compound_destress = FALSE;
				break;
			...
			case QUEST:          			
				...
				pDph_t->clausenumber=0;
				add_feature (pDph_t,FQUENEXT, (short)(CURRPHONE) ); 
				last_bound = 0;
				if(pKsd_t->lang_curr == LANG_latin_american
					|| pKsd_t->lang_curr == LANG_spanish )
				{
...
#ifdef MSDBG8
				FNX_PRINTF ("add_feature(FBLOCK,NEXTPHONE)\n");
#endif
				break;

			case NEW_PARAGRAPH:
				add_feature (pDph_t, PRESSBOUND, NEXTPHONE);
				add_feature (pDph_t, PRESSBOUND, (short)(pDph_t->nphonetot+1));
			...
			default:
...
#ifdef MSDBG8
						FNX_PRINTF ("add_feature(FSTRESS_1,NEXTPHONE)\n");
#endif
						break;
					case S2:
...
#ifdef MSDBG8
						FNX_PRINTF ("add_feature(FSTRESS_2,NEXTPHONE)\n");
#endif
						break;
...
#ifdef MSDBG8
						FNX_PRINTF ("add_feature(FEMPHASIS,NEXTPHONE\n");
				...
				else
				{
					...
					case WBOUND:
						...
						vowel_count = 0;
						word_init_sw = TRUE;
						add_feature (pDph_t, FWBNEXT, (short)(NEXTPHONE) );
					...
					case STRONG_RISE:
						...
						//	}
						//else 
						//above out because dectalk promotes stress marker to next syllll
						{
							if (curr_in_sym == STRONG_RISE) {
								add_feature_j(pDph_t,FSTRONG,pDph_t->nphonetot);
							}
							else
								add_feature_j(pDph_t,FRISE,pDph_t->nphonetot);		
							break;
						}

					case ACCENT_FALL:
						if ((phone_feature( pDph_t,pDph_t->symbols[n-1]   ) & FSYLL)IS_PLUS)
							add_feature_j(pDph_t,FFALL,(short)(pDph_t->nphonetot-1));
						else
							//must be a cvc
							add_feature_j(pDph_t,FFALL,(short)(pDph_t->nphonetot-2));
						break;

					case STRONG_FALL:

						add_feature_j(pDph_t,FSTRONG,(short)(pDph_t->nphonetot-1));
						add_feature_j(pDph_t,FFALL,(short)(pDph_t->nphonetot-1));
						break;


					case LONG_PHONE:
						add_feature_j(pDph_t,FLONG,(short)(pDph_t->nphonetot-1));
						break;
					...
					default:
							...
							&& ( (phone_feature( pDph_t, curr_in_sym) & FVOWEL) IS_MINUS) )
						{
							...
							if (struc[pDph_t->nphonetot] != 0) {	 /* Absorb cur feat */
								add_feature_j(struc[pDph_t->nphonetot],pDph_t->nphonetot-1);
...
#else
			adjust_index (pKsd_t, (n ), -1, 0);	/* eab n+1 to bind forward */
#endif
#ifdef DEBUGIND

			FNX_PRINTF ("adj -1 on %d %d \n", curr_in_sym, (n + 1));
#endif
		}
	}
	return (TRUE);
...
#else
		adjust_index (pKsd_t, (loc ), 1, 0);
#endif
	}
#ifdef DEBUGIND
	FNX_PRINTF ("loc %d pDph_t->nsymbtot %d \n", loc, pDph_t->nsymbtot);
	FNX_PRINTF ("adj +1 inserting %d \n", fone);
#endif
	return;
}
 
...
#ifdef DEBUGIND
	FNX_PRINTF ("adj -1 del sym %d at %d \n ", pDph_t->symbols[msym], msym);
#endif
	pDph_t->nsymbtot--;
...
void add_feature (PDPH_T pDph_t, long feaname, short location)
{
	...
	if ((pDph_t->sentstruc[location] & FBOUNDARY) < (U32)(feaname & FBOUNDARY))
...
//	printf(" sent %d loc %d fea %d \n",pDph_t->sentstruc[location],location,feaname);
...
void find_syll_to_stress (LPTTS_HANDLE_T phTTS, short *locend, short nstartphrase, short nstresses)
{
	...
	for (m = *locend - 1; m >= nstartphrase; m--)
	{
		if (pDph_t->symbols[m] == S1)
			return;
		if (pDph_t->symbols[m] == S2 && nstresses > 1)
		{
	...
	for (m = locbeg; m < *locend; m++)
	{
		...
		if ((phone_feature(pDph_t,pDph_t->symbols[m]) & FSYLL) IS_PLUS)
		{
			...
			insertphone (phTTS, m, S2);		/* Insert S2 at location m */
...
#ifndef AD_VDSP
__inline int phone_feature(PDPH_T pDph_t, int phone)
{
