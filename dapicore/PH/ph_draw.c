...
int phone_feature_2(PDPH_T pDph_t,int a)
{
	if (pDph_t->allofeats[(a)] & FDUMMY_VOWEL)
	{
		if (phone_feature(pDph_t,pDph_t->allophons[(a-1)]) & FVOICD)
		{
			return (phone_feature(pDph_t,pDph_t->allophons[(a)]));
		}
		else
		{
			return (phone_feature(pDph_t,pDph_t->allophons[(a)])&(~FVOICD));
		}
	}
	else
	{
...
void phdraw (LPTTS_HANDLE_T phTTS)
{
	...
	short	final_area_rel[18] = { 101,151, 181, 240,260,
		280,310,450,500,600,700,1000,1000,
		1000,1000,1000,1000,1000};
...
//		printf("begin tcum %d out_a2=%d\n",pDph_t->tcum,(pDph_t->parstochip[OUT_A2]));
		...
		*/
		...
		for (; np <= &PTILT; np++)
		{
			...
			*/
			if (pDph_t->tcum >= np->tbacktr)
			{
				if (np==&PAP && pDph_t->allofeats[pDph_t->nphone ] & FDUMMY_VOWEL)
				{
					*parp=value;
				}
				else
			...
			if(np == &PAV)
			{
				...
				if(pDph_t->vot)
				{
					if(pKsd_t->lang_curr == LANG_french)
					{
						...
						pDph_t->target_ag = 0;
						if(  pDph_t->tcum == pDph_t->vot-1)
						{
							pDph_t->target_ag = 120;
						}
						else if(  pDph_t->tcum >= pDph_t->vot)
						{
							pDph_t->target_ag = pVtm_t->NOM_VOIC_GLOT_AREA;
						}
						PH_DEBUG_2080_P1(" vot sets glotal opening %d \n",pDph_t->vot);
					}
					else
					{
						...
						if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FSON1)
						{
							...
							else if( pDph_t->tcum == pDph_t->vot-5 )
							{
								if(pVtm_t->VOT_speed)
								{
									pDph_t->agspeed = pVtm_t->VOT_speed;
							...
							else if( pDph_t->tcum == pDph_t->vot-2 && 
							...
							if( pDph_t->tcum == pDph_t->vot )
							{
								...
								PH_DEBUG_2080_P2(" vot sets glottal closing: vot=%d, target_ag=%d \n",
									pDph_t->vot, pDph_t->target_ag);
							}
						}	
					}

#if ( defined INTEGER && !defined OLD_WAY )  //remoive for old av rules.
					// Set AF, AH and AV based on target_ag

					if( pDph_t->target_ag < pVtm_t->NOM_VOIC_GLOT_AREA )
					{
						pDph_t->parstochip[OUT_AV] = 65;
						pDph_t->parstochip[OUT_GF] = 0;
					}
					else if( pVtm_t->NOM_VOIC_GLOT_AREA <= pDph_t->target_ag &&
							 pDph_t->target_ag <= pVtm_t->NOM_UNVOICED_SON )
					{
						if( np->pspesh == 0 )						// Initialize if just getting started
						{
							pDph_t->parstochip[OUT_AV] = np->tarcur - (np->tarcur >> 2);
						}
						else if( (np->tarcur - np->pspesh) <= 2 )	// Very close to target so just set it to target
						{
							pDph_t->parstochip[OUT_AV] = np->tarcur;
						}
						else if( np->pspesh < np->tarcur )			// Go half way to target
						{
							pDph_t->parstochip[OUT_AV] = np->pspesh + ((np->tarcur - np->pspesh) >> 1);
						}
						np->pspesh = pDph_t->parstochip[OUT_AV];	// Store value for next time around
					}
					else
					{
...
//	FNX_PRINTF( " %d \n",pDph_t->parstochip[OUT_AV]);
	...
	*parp += (pDph_t->spdeftltoff - 3); /*from fphdraw.c*/
							...
							if(!(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FPLOSV))
							{
								...
								if(!(place(pDph_t->allophons[pDph_t->nphone+1]) & FLABIAL))
								{
									...
									if (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FVOICD)
										pDph_t->target_l= pVtm_t->NOM_Fricative_Opening+50;
									else
				...
				if (pDph_t->tcum >= np->tspesh )
				{
					...
					{	
						...
						else	//affricate- release it
						{
								...
								// Don't change the target if it's already set for frication
								//if( !(70 <= pDph_t->target_b && pDph_t->target_b <= 200) )	
								{
									if(phone_feature_2(pDph_t,pDph_t->nphone) & FVOICD)
									{
										pDph_t->target_b = pVtm_t->NOM_Fricative_Opening+30;
									}
									else
									pDph_t->target_b = pVtm_t->NOM_Fricative_Opening;
						...
						if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FPLOSV)
						{
							...
							if(!(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FSTOP))
							{
								...
								if(!(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FOBST))
								{
									...
									pDph_t->tbstep = -1;
									pDph_t->target_tb = pVtm_t->NOM_Fricative_Opening+20;
								}
								else
								{
		...
		( pDph_t->nphone > pDph_t->nallotot -4)	)
	{
		...
		pDph_t->target_ap =0;
#ifdef INTEGER
		// We need to do this to make sure that any phrase final voicing gets shut off - test word: "broken"
		pDph_t->target_ag = pVtm_t->NOM_UNVOICED_SON;
#endif
		}
...
#ifdef HLSYN		
		//moved this down due to dependency on area_n
		//Do_ue_dc(phTTS); 
	...
	
	{
		...
		if(DT_DBG(PH_DBG,0x080))
		{
			FNX_PRINTF("opening glottis later after nasal for unvoiced obst stop\n");
		}
#endif
		pDph_t->target_ag = pVtm_t->NOM_Open_Glottis;
		if( pDph_t->nphone > 0 &&
	...
	else
	{
		...
		coart_transtime=0;
		if (coarticulation > (pDph_t->allodurs[pDph_t->nphone]*1)>>1)
			coarticulation=(pDph_t->allodurs[pDph_t->nphone]*1)>>1;
		else
			coarticulation=4;
	}
	...
	if (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FOBST)
					  ...
					  )
					{
							...
							pDph_t->allophons[pDph_t->nphone] == USP_JH)
						{
							...
							if(  pDph_t->localcnt <= 5 )
							{
								...
								PH_DEBUG_2080(" PALATEL roll start \n ");
							}
							else
							{
								pDph_t->parstochip[OUT_A2] = 2200;	
						...
						{		
							...
							PH_DEBUG_2080(" PALATEL or palatel-alvelar affected so jam A2\n");
			...
			(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FOBST))
		{
				...
				
			{
				...
				if(DT_DBG(PH_DBG,0x080))
				{
					FNX_PRINTF(" coarticulating A2 with lastthing %d\n",pDph_t->parstochip[OUT_A2]);
				}
#endif
			}
			else
			{
					...
					&& pDph_t->phonestep < pDph_t->allodurs[pDph_t->nphone] >>1)
				{
					...
					if(DT_DBG(PH_DBG,0x080))
					{
						FNX_PRINTF(" coarticulating A2 with lastthing-2 %d\n",pDph_t->parstochip[OUT_A2]);
					}
#endif
...
#ifdef HLSYN
	// moved this here because is depends on the value of area_n
	Do_ue_dc(phTTS);		
#endif
	...
	{
#if 0
		// This over-rides the vot onset, which is bad
		if( pDph_t->target_ag > pVtm_t->NOM_VOIC_GLOT_AREA+800 &&
			pKsd_t->lang_curr == LANG_korean )						// For other languages this can make p,t & k sound like b, d & g
		{
			// We have a long ways to go in only 3 frames so close half the distance
			pDph_t->target_ag -= (pDph_t->target_ag - pVtm_t->NOM_VOIC_GLOT_AREA)>>1;
		}
#endif
		...
		PH_DEBUG_2080_P1(" Anticipating fson1 so target_ag = %d glottis  \n",pDph_t->target_ag);
	}
	...
	if(phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone]) & FOBST )
	{
				 ...
				 )
		{
			...
			if(pDph_t->target_ag >= pVtm_t->NOM_VOIC_GLOT_AREA + 800)
			{
				...
				else if( (phone_feature_2(pDph_t,pDph_t->nphone) & FVOICD) )
				{
					pDph_t->agspeed=1;
					PH_DEBUG_2080_P1(" Voiced Obstruent, use high agspeed %d\n", pDph_t->agspeed);
				}
				else
				{
					pDph_t->agspeed=3;
					...
					(phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone]) & FOBST) && 
					pDph_t->nphone > 0 &&
					(phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone-1]) & FVOWEL) && 
					(phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone+1]) & FVOWEL) )
			{
				pDph_t->agspeed=1;
				PH_DEBUG_2080_P2(" Voiced obstruent preceded and followed by a vowel, agspeed = %d (target_ag=%d)\n", 
					pDph_t->agspeed, pDph_t->target_ag);
			}
			else if((phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone]) & FOBST) 
				&& (phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone]) & FSONOR)
				&& pDph_t->nphone > 0
			...
			{
				pDph_t->agspeed=1;
				PH_DEBUG_2080_P1(" voicd obst plosv to Obstr Sonor agspeed = %d\n", pDph_t->agspeed);
			}
			else
			{
				pDph_t->agspeed=3;
				PH_DEBUG_2080_P1(" Obstr to voiced agspeed = %d\n", pDph_t->agspeed);
			}
		}
	}


	//printf("mid4 tcum %d out_a2=%d\n",pDph_t->tcum,(pDph_t->parstochip[OUT_A2]));
		...
		&& pDph_t->nphone != 0)
	{
		...
		{
			pDph_t->pressure += 70;
			PH_DEBUG_2080_P1(" Raising sub-glottal pressure (sub-press=%d)\n.", 
				pDph_t->pressure);
		}
		else if( pDph_t->pressure > pVtm_t->NOM_Sub_Pressure+70 )
		{
			pDph_t->pressure -= 100;
			PH_DEBUG_2080_P1(" Lowering sub-glottal pressure (sub-press=%d)\n.", 
				pDph_t->pressure);
		}
	}
	else if( pKsd_t->lang_curr == LANG_korean &&
			 pDph_t->pressure <= pVtm_t->NOM_Sub_Pressure+400 &&
			 (pDph_t->allophons[pDph_t->nphone] == KRP_PP ||
			  pDph_t->allophons[pDph_t->nphone] == KRP_TT ||
			  pDph_t->allophons[pDph_t->nphone] == KRP_KK ||
			  pDph_t->allophons[pDph_t->nphone] == KRP_SS ||
			  pDph_t->allophons[pDph_t->nphone] == KRP_CC) )
	{
		pDph_t->pressure += 100;
		PH_DEBUG_2080_P1(" Raising sub-glottal pressure for tense plosive (sub-press=%d)\n.", 
			pDph_t->pressure);
	}
	...
	{		
			...
			pDph_t->allophons[pDph_t->nphone] != GEN_SIL)
		{
			...
			{	
				...
				// Speed up the process of closing the glottis if needed.
				if( pDph_t->nphone > 0 &&
					phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1]) & FBURST &&
					phone_feature_2(pDph_t,pDph_t->nphone) & FVOICD &&
	...
	{	
		...
		{	
			...
			{	//plosive release sits stays at fric value until one step after bstep =0
				if(pDph_t->lstep == -1) //HELPME MGS
				{
					if(phone_feature_2( pDph_t, pDph_t->nphone) & FVOICD)
					{
						pDph_t->area_l = pVtm_t->NOM_Fricative_Opening+50;
						pDph_t->target_l = pVtm_t->NOM_Fricative_Opening+50;
					}
					else
					{
				...
				else
				{
					...
					{
						pDph_t->area_l += 50;
						//pDph_t->target_l+=50; // without this  values go 150 100 400
					}
		...
		else if((pDph_t->in_lrelease == 0 || pDph_t->in_lclosure == 0))
		{
			...
			//than closing
			// MGS what is this line doing? why is it here? 
			//seems like it wouldn't be able to close if this is here
//			pDph_t->area_l += ((pDph_t->target_l - pDph_t->area_l)>>2);
	...
	if(pDph_t->target_b < pDph_t->area_b)
	{
		if(pDph_t->area_b)
		{
				...
				(phone_feature_2(pDph_t,pDph_t->nphone-1) & FVOICD))
			{
				...
				// if it is a nasal, make the last step before close  very small opening to avoid click
				if (pDph_t->area_b <=50)
	...
	{	
		if(pDph_t->in_brelease == 1 && pDph_t->area_b < 1000)
		{
			if( (pDph_t->nphone > 0 && 
				 (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1])) & FBURST) ||
				(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone])) & FBURST	)
			{	//plosive release stays at fric value until one step after bstep =0
				if(pDph_t->bstep < 0)// HELPME MGS
				{
					...
					else
					{
						...
						{
//							pDph_t->target_b = pVtm_t->NOM_Fricative_Opening+30;
							pDph_t->area_b = pVtm_t->NOM_Fricative_Opening+30;
						}
						else
				...
				PH_DEBUG_2080_P1(" Phone with burst with blade affected so area_b = %d\n", pDph_t->area_b);
	...
	if( pDph_t->nphone > 0 &&
		place(pDph_t->allophons[pDph_t->nphone-1]) & BLADEAFFECTED && 
		// dont coarticulate if the previous was a stop
	...
	if( pDph_t->nphone > 0 &&
		...
		&& pDph_t->nphone !=0)
	{
		if(pDph_t->phonestep <  4 && pDph_t->area_n>0)//(coarticulation-coart_transtime))
		{
			pDph_t->area_b = pDph_t->last_area_b;
#ifdef PH_DEBUG
			if(DT_DBG(PH_DBG,0x080))
		...
		else if (pDph_t->phonestep <  coarticulation+2)
		{
			pDph_t->area_b = area_rel[pDph_t->phonestep-3];
			...
			&& pDph_t->tcum >=7)
			{
				...
				if(place(pDph_t->allophons[pDph_t->nphone+1]) & FLABIAL
					|| (place(pDph_t->allophons[pDph_t->nphone+1]) & FDENTAL ))
						...
						if(DT_DBG(PH_DBG,0x080))
						{
							FNX_PRINTF("Set at for next consonant A2=%d  \n",pDph_t->parstochip[OUT_A2]);
						}
#endif
...
//				&& (pDph_t->nallotot - pDph_t->nphone) <= 10)
			...
			if( pDph_t->nphone  >= pDph_t->last_sonorant
				&& ((pDph_t->last_sonorant > 1) || (pDph_t->last_sonorant >(pDph_t->nallotot-5))) && 
				pDph_t->nphone > (pDph_t->nallotot -7) )
				//&& pDph_t->number_words > 1 )					// Don't do end of phrase effects if there's only 1 word
			{
				if (pDph_t->end_of_phrase_step==0)
				{
				//	if (pDph_t->nphone == pDph_t->last_sonorant &&
				//		((pDph_t->tcum>=(pDph_t->allodurs[pDph_t->nphone]>>1)) ||
				//		(pDph_t->nphone==pDph_t->nallotot-2)))
					{
						pDph_t->end_of_phrase_dur=0;
						for (tmp1=pDph_t->nphone;tmp1<=pDph_t->nallotot-2;tmp1++)
						{			
							if((pDph_t->allofeats[pDph_t->nphone ] & FDUMMY_VOWEL))
								pDph_t->end_of_phrase_dur+= (pDph_t->allodurs[tmp1]>>1);
							else
								pDph_t->end_of_phrase_dur+=pDph_t->allodurs[tmp1];

						}
						pDph_t->end_of_phrase_dur-=(pDph_t->tcum-1);
						//if(pVtm_t->EndOfPhrase_Spread_target) 
							// hard = 
							//pDph_t->last_real_phon == d

								/// d	pressure	spread
							//			550			353
							//			
							//
							//pVtm_t->EndOfPhrase_Spread_target = 503;

						pDph_t->end_of_phrase_step=(pVtm_t->EndOfPhrase_Spread_target)/pDph_t->end_of_phrase_dur;
						
						if (pDph_t->number_words==1 || 
								(phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nallotot-2]) & FOBST 
								&& (phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nallotot-2]) & FCONSON)
								&& phone_feature_2(pDph_t, pDph_t->nallotot-2) & FVOICD ))
						{
							// if there is only 1 word, do half the speading.
							pDph_t->end_of_phrase_step>>=1;
							if (pDph_t->end_of_phrase_step<1)
								pDph_t->end_of_phrase_step=1;
						}
						...
						if(DT_DBG(PH_DBG,0x080))
						{
							FNX_PRINTF(" Spreading  glotis at final sonorant phone %d spread_step %d target %d dur %d\n",pDph_t->nphone,pDph_t->end_of_phrase_step,pVtm_t->EndOfPhrase_Spread_target,pDph_t->end_of_phrase_dur);
				...
				else
				{
					...
					{
						pDph_t->delta_area_g+=pDph_t->end_of_phrase_step;
					}
					else
					{
						pDph_t->delta_area_g+=pVtm_t->EndOfPhrase_Spread>>2;
					}
#ifdef PH_DEBUG
					if(DT_DBG(PH_DBG,0x080))
					{
						FNX_PRINTF(" Spreading  glotis at final sonorant phone %d delta_area_g value %d and pressure_droping also %d target ag %d\n",pDph_t->nphone,pDph_t->delta_area_g,pDph_t->pressure_drop,pDph_t->target_ag);
...
//						&& (pDph_t->tcumdur > 5)
{
	...
	if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FSYLL)
	{
			...
			pDph_t->allophons[pDph_t->nphone+1]==GEN_SIL)
		{
			...
			PH_DEBUG_2080_P1("dummy vowel pressure_drop is now %d\n.",pDph_t->pressure_drop);
		...
		if(pDph_t->allofeats[pDph_t->nphone ] & FDUMMY_VOWEL)
				...
				if( pDph_t->syl_pressure != 0)
				{
					...
					PH_DEBUG_2080_P2("adding syl_pressure %d to pressure_drop %d\n.",pDph_t->syl_pressure,pDph_t->pressure_drop);
				}
#endif
...
#ifdef HLSYN		
		// Add diplophonic pulsing (creak) at end of utterance 
		...
		if( (pDph_t->area_l >= 150 && 
			 pDph_t->area_b >= 150 &&
			 pDph_t->area_tb >= 150) || 
			pDph_t->area_n > 150 || 
			pDph_t->nphone == 0 )
			{
				// opening is large enough that all pressure leaked out
				...
				pDph_t->area_tb != 0 )
			{
				...
				if( pDph_t->area_l < iMinArea )
					iMinArea = pDph_t->area_l;
				if( pDph_t->area_b < iMinArea )
					iMinArea = pDph_t->area_b;
				if( pDph_t->area_tb < iMinArea )
					iMinArea = pDph_t->area_tb;

				//for very small areas reduce the gf
				if(iMinArea <= 10)
					tmp = 37;
				else if(iMinArea <= 20)
					tmp = 40;
					...
					tmp = 47;
				else if(iMinArea < 100)
					tmp = 50;
				else
				{
					// This is a heuristic equation derived from the High Compute data
					//tmp = ((1000-iMinArea)*47 / 600) - 13;
					tmp = ((1000-iMinArea)*40 / 600) - 10;		// iMinArea=700, tmp = 10 : iMinArea = 100, tmp = 50
				}
#else
					if(pDph_t->area_l < 80)
						tmp1=pDph_t->area_l;
			...
			//if (np==&PAV)
			{
				// Set AF, AH and AV based on area_g
				int cutoff=pVtm_t->NOM_UNVOICED_SON-300;
				if( (pDph_t->area_l == 0 ||		// Can't have any noise coming out if the
					 pDph_t->area_b == 0 ||		// vocal tract is completely shut
					 pDph_t->area_tb == 0) &&
					pDph_t->area_n == 0)
				{
					pDph_t->parstochip[OUT_AV] = 0;
				...
				else if( pDph_t->area_g <= pVtm_t->NOM_VOIC_GLOT_AREA )
				{
					pDph_t->parstochip[OUT_AV] = 65;
					pDph_t->parstochip[OUT_GF] = 0;
				}
				else if( pDph_t->area_g > cutoff )
				{
					pDph_t->parstochip[OUT_AV] = 0;
				}
				else if ( pVtm_t->NOM_VOIC_GLOT_AREA < pDph_t->area_g &&
						  pDph_t->area_g <=  cutoff )
				{
					int t1;

					t1=(27-65)*pDph_t->area_g-(27-65)*pVtm_t->NOM_VOIC_GLOT_AREA;

					//t1= cutoff-pDph_t->area_g;
					//t1*=65;
					t1/=(cutoff-pVtm_t->NOM_VOIC_GLOT_AREA);
					t1+=65;
					
					pDph_t->parstochip[OUT_AV]= t1;
					
				}
				
				if (pDph_t->nphone==0)
				{
					pDph_t->parstochip[OUT_AV]>>=(2-pDph_t->tcum);
				}
				else if ((&PAV)->pspesh==0)
				{
					pDph_t->parstochip[OUT_AV]>>=1;
				}
				(&PAV)->pspesh = pDph_t->parstochip[OUT_AV];	// Store value for next time around
			}
...
//	FNX_PRINTF( " %d \n",pDph_t->parstochip[OUT_AV]);
	...
	if	((pDph_t->allofeats[pDph_t->nphone ] & FDUMMY_VOWEL) || (pDph_t->allofeats[pDph_t->nphone-1 ] & FDUMMY_VOWEL) )
			{
				if (pDph_t->allofeats[pDph_t->nphone-1 ] & FDUMMY_VOWEL)
				{
					factor=pDph_t->phonestep+pDph_t->allodurs[pDph_t->nphone-1 ];
				}
				else
				{
					factor=pDph_t->phonestep;
				}

				{
					if (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-2]) & FVOICD)
					{
						if (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1]) & FVOICD)
						{
							pDph_t->parstochip[OUT_AP] -= factor;  //voiced voiced
						}
						else
						{
							if (pDph_t->allophons[pDph_t->nphone-1]==USP_K)
								pDph_t->parstochip[OUT_AP] -= factor<<1; //voiced unvoiced		
							else
								pDph_t->parstochip[OUT_AP] -= factor<<2; //voiced unvoiced
						}
					}
					else
					{
						if (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1]) & FVOICD)
						{
							pDph_t->parstochip[OUT_AP] -= factor; // unvoiced voiced
						}
						else
						{
							pDph_t->parstochip[OUT_AP] -= factor<<2; // unvoiced unvoiced
						}
					}
					if(pDph_t->parstochip[OUT_AP] < 0)
						pDph_t->parstochip[OUT_AP] =0;
				}
				//else
				
				{
					pDph_t->parstochip[OUT_AV] -= factor<<2;
			...
			if((pDph_t->area_g + pDph_t->delta_area_gst - pDph_t->delta_area_gstop) < 1200)
			{
				...
				tmp -= 5;
				if((pDph_t->area_l <= 250 || 
					pDph_t->area_b <= 250 || 
					pDph_t->area_tb <= 250) && 
				   pDph_t->area_n < 100)
				{
					pDph_t->av_reduction += 4;
					if( pDph_t->av_reduction > 8 )
						pDph_t->av_reduction = 8;
				}
				else if( pDph_t->av_reduction > 0 )
				{
					pDph_t->av_reduction -= 4;
					if( pDph_t->av_reduction < 0 )
			...
			if	((pDph_t->allofeats[pDph_t->nphone ] & FDUMMY_VOWEL) || (pDph_t->allofeats[pDph_t->nphone-1 ] & FDUMMY_VOWEL) )
			{
				if (((pDph_t->allofeats[pDph_t->nphone ] & FDUMMY_VOWEL) && (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1]) & FVOICD)) || 
					((pDph_t->allofeats[pDph_t->nphone-1 ] & FDUMMY_VOWEL) && (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-2]) & FVOICD)))
				{
					pDph_t->parstochip[OUT_GF] -= (factor<<1);
				}
				else
				{
					pDph_t->parstochip[OUT_GF] -= (factor<<2);
				}
					if(pDph_t->parstochip[OUT_GF] < 0)
						pDph_t->parstochip[OUT_GF] =0;
			}

			// We should still have voicing if the previous phone was voiced 
			// and the area of the glottis is still small enough for voicing
			// and there isn't a small constriction
#if 0
			if( pDph_t->nphone > 0 &&
				!(pDph_t->allofeats[pDph_t->nphone] & FDUMMY_VOWEL) &&
				pDph_t->allophons[pDph_t->nphone] != GEN_SIL &&
			...
			//FNX_PRINTF("tmp %d press %d press_drop %d press_pulse %d  perss_gest %d hi \n",tmp,pDph_t->pressure , pDph_t->pressure_drop , pDph_t->stress_pulse, pDph_t->pressure_gest);
			if(pDph_t->nphone >=1)
			{
			...
			//FNX_PRINTF("tmp %d press %d press_drop %d press_pulse %d  perss_gest %d pburst %dhi \n",tmp,pDph_t->pressure , pDph_t->pressure_drop , pDph_t->stress_pulse, pDph_t->pressure_gest, pDph_t->pressure_burst);
			//FNX_PRINTF("%d pburst %d %d %d \n",pDph_t->pressure, pDph_t->pressure_burst,tmp, pDph_t->nphone);
				...
				//	pDph_t->parstochip[OUT_FNP] = 500;
...
#else
						if( pDph_t->nphone > 0 &&
							(phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone-1)]) & FVOWEL) IS_PLUS)
						{
							 ...
							 pDph_t->allophons[pDph_t->nphone] == KRP_DF )
					{
						...
						*pDph_t->param[AREAB].outp = 600;
						...
						// Keep area_b and target_b up-to-date so that we will transition
						// properly to the next phoneme.
						pDph_t->area_b = *pDph_t->param[AREAB].outp;
						pDph_t->target_b = *pDph_t->param[AREAB].outp;
#else
						short 
							pwSequence[] = {
							0,0,0,0											// voicing
							-N75PRCNT,-N95PRCNT, 0,							// tap
							0,0,0,0};										// voicing
						int
							nSeq = sizeof(pwSequence) / sizeof(short);

						// Artificially hold tongue blade open to avoid frication
						pDph_t->area_b = 1000;
						pDph_t->target_ag = pVtm_t->NOM_VOIC_GLOT_AREA+100;
						pDph_t->parstochip[OUT_A2] = 2100;

						ParameterModulation(phTTS, &pDph_t->area_b, pwSequence, nSeq, 
							pDph_t->smodulcount < nSeq-1 ? TRUE : FALSE);	// Don't increment smodulcount once we hit the end of pwSequence
						pDph_t->target_b = pDph_t->area_b;
				...
				if (pKsd_t->lang_curr == LANG_french)
					if(pDph_t->parstochip[OUT_AV] > 10)
...
void Phrase_intial_sil_start(LPTTS_HANDLE_T phTTS)
{
	...
	if(pDph_t->nphone != pDph_t->nphonelast)
	{
		...
		if(pDph_t->number_words == 1)
		{
			...
			if( (phone_feature(pDph_t,pDph_t->allophons[1]) & FPLOSV) IS_PLUS
				&& (phone_feature_2(pDph_t,1) & FVOICD) IS_MINUS)
			{
				pDph_t->pressure = 300;
	...
	{	
		...
		if (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FOBST)
		{
				...
				phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FPLOSV)
			{
				if( pKsd_t->lang_curr == LANG_spanish ||
					pKsd_t->lang_curr == LANG_latin_american )
				{
					pDph_t->target_ag = 600;		// Start voicing a little sooner for spanish
					pDph_t->area_g = 700;			// so we have a strong voicebar for voiced stops
				}
				else
				{
		...
		else if((place(pDph_t->allophons[pDph_t->nphone+1])) & FLABIAL )		// FVOICD + FLABIAL
		{	
			pDph_t->in_lclosure = 0;
			pDph_t->area_l = pVtm_t->NOM_Fricative_Opening;
			pDph_t->area_b = 1000;

			pDph_t->target_l=150; // this was 150, causing the sequence 150 100 400 in [b ll]
			pDph_t->target_narea =0;
			pDph_t->lstep=0;
...
void Do_ue_dc(LPTTS_HANDLE_T phTTS)
{
	...
	if ((phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FSON1) &&
		(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1]) & FNASAL) &&
		pDph_t->area_n == 0 && pDph_t->last_area_n!=0
		&& pDph_t->dcstep==0)
	{
		pDph_t->dcstep=-1;
		pDph_t->uestep=-1;
	...
	if((phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FOBST) 
		&& pDph_t->dcstep == 0
		&& pDph_t->area_n == 0
		&& !( phone_feature( pDph_t, pDph_t->allophons[ pDph_t->nphone])& FBURST		// Not an affricate
		&&	  !(phone_feature( pDph_t, pDph_t->allophons[ pDph_t->nphone])& FPLOSV) )
		//&& pDph_t->phonestep >= 1  // why phonestep >=1?? why dont do step 0?
		&& (pDph_t->allophons[ pDph_t->nphone] != SPP_BH)		// Not a Spanish approximate
		...
		)
	{
		...
		if( phone_feature_2( pDph_t, ( pDph_t->nphone)) & FVOICD
			&& !(phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone)]) & FNASAL))
		{
			// block short duration movements of ue and dc
			// don't open ue and dc if this phoneme is a stop and the previous is a stop
			if ((phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone)]) & FSTOP) &&
				pDph_t->allodurs[pDph_t->nphone]-pDph_t->tcum<7)
			{
				if( pDph_t->nphone > 0 &&
					(phone_feature( pDph_t, pDph_t->allophons[pDph_t->nphone-1]) & FNASAL) &&
					(pDph_t->last_area_n))
					// and after nasal
				{
					//add condition for variable on this
				...
				else if (pDph_t->allodurs[pDph_t->nphone]-pDph_t->tcum<4)
				{
				// block dc and ue for short durations.
				}
				else
				{
					// move faster for short durations
				// block dc and ue for short durations.
					pDph_t->dcstep=2;
					pDph_t->uestep=2;
				}
	...
	if(pDph_t->dcstep != 0	&& just_set_it==0)
	{
			...
			pDph_t->allophons[ pDph_t->nphone] == GEN_SIL )
		{
			...
			else if ((phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone)]) & FSON1))
			{
				...
				if (pDph_t->area_n!=0)
						// reduce frication immediatly after the nasal
				{
					pDph_t->dcstep=0;
					pDph_t->uestep=0;
				}
				...
				&& phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone)]& FOBST))))
			{
				...
				// move dcstep back towards 0 at the end of a burst/stop or 
				// at the beginning of a sonorant except if the language is Spanish.
				// In Spanish we want a stronger voicebar
				if( ((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FBURST) &&		// Last 3 frames of a burst
					 (pDph_t->tcum >= pDph_t->allodurs[pDph_t->nphone] - 3) &&
					 ((pKsd_t->lang_curr != LANG_spanish &&
					 pKsd_t->lang_curr != LANG_latin_american )) ||
					 ((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone-1]) & FNASAL)))
					|| 
					((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSON1) &&
					 (!(phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FOBST)) && // First 6 frames of a FSON1
					  pDph_t->phonestep < 6) )
				{
					if(pDph_t->dcstep > 1)
					{
		...
		pKsd_t->lang_curr == LANG_latin_american)
	{
		if( pDph_t->nphone > 0 &&
			(phone_feature( pDph_t, pDph_t->allophons[pDph_t->nphone-1]) & FNASAL) &&
			(pDph_t->area_n==0 && pDph_t->last_area_n))
		{
			// block this reduction for 1 frame after the velum closes 
		}
		else
		{
...
void Set_pressure(LPTTS_HANDLE_T phTTS)
{
	...
	if(pDph_t->allophons[ pDph_t->nphone] == GEN_SIL)
	{
		if ((pDph_t->nphone == 0 || pDph_t->nphone>pDph_t->nallotot-2))
		{
	...
	if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FOBST)
	{
		if((phone_feature_2(pDph_t,pDph_t->nphone) & FVOICD))
		{
			...
			else
			{
					...
					phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FPLOSV)
				{
						...
						pKsd_t->lang_curr == LANG_latin_american )
					{
						if( pDph_t->nphone > 0 &&
							pDph_t->allophons[pDph_t->nphone-1] == GEN_SIL )
						{
							pDph_t->target_ag = 600;	// open glottis enough for voicing
							pDph_t->area_g=600;
							pDph_t->target_ap = 0;		// close chink

							PH_DEBUG_2080(" Spanish phrase initial voicebar\n");
						}
						else if ( pDph_t->nphone > 0 &&
							phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1]) & FNASAL)
						{
							...
							PH_DEBUG_2080(" Spanish voicebar after nasal (open glottis and close chink) ...\n");
						}
						else
						{
							pDph_t->target_ag = 600;	// open glottis enough for voicing
							...
							PH_DEBUG_2080(" Spanish voicebar (open glottis and close chink) ...\n");
						}
					...
					else
					{
						if( pDph_t->nphone > 0 &&
							phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1]) & FNASAL)
						{
							// previous phone is a nasal, open the glottis slower
							pDph_t->target_ag = 400;	// area of glottis
							
							pDph_t->target_ap = 0;		// chink
							PH_DEBUG_2080(" Voicebar ... after nasal, use velum for voicebar\n");
						}
						else
						{
				...
				if( pDph_t->allophons[pDph_t->nphone] == KRP_JH )
				{
					pDph_t->target_ag = pVtm_t->NOM_VOICED_OBSTRUENT-300;	//-400;	
					//... add a little more to soften?					
					//pDph_t->target_b  = pVtm_t->NOM_Fricative_Opening;
					pDph_t->target_ap = 0;
					PH_DEBUG_2080_P2(" KRP_JH - target_ag=%d, target_b=%d\n", pDph_t->target_ag, pDph_t->target_b);
				}
...
//					pDph_t->area_ap+=100;
					...
					PH_DEBUG_2080_P2("unoiced obstruent non-stop after nasal so move glottis to %d and ap to %d faster to avoid strong voicing\n",
							pDph_t->target_ag,pDph_t->target_ap);
				}

				PH_DEBUG_2080_P2("Voiced obstruent so glottis to %d and ap to %d\n",
					pDph_t->target_ag, pDph_t->target_ap);
		...
		else
		{
			...
			else
			{
				...
				if (((phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone]) & FBURST)
					...
					&& (phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone-1]) & FNASAL))
				{
					//delay opening until later if after nasal
...
void Do_Nasal(LPTTS_HANDLE_T phTTS)
{
	...
	if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FNASAL )
	{
		...
		{
			if ((pDph_t->allodurs[pDph_t->nphone] - pDph_t->tcum) <= 2)
			{
			...
			(pDph_t->allodurs[pDph_t->nphone] - pDph_t->tcum) <= 3   )				// Near end of nasal
		{
			...
			PH_DEBUG_2080(" Nasal --> voiced obst non-stop so shut velum early\n");
		}
		else if(pDph_t->nasal_step < 7)
		{
			if (pDph_t->allophons[pDph_t->nphone]==pDph_t->allophons[pDph_t->nphone+1])
			{
				pDph_t->nasal_step++; // slower opening in homo-organic nasal-nasal
		...
		phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FSON1 ) 
	{
		...
		if (!(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FOBST))
		{
			...
			if(pDph_t->allophons[pDph_t->nphone+1] != pDph_t->allophons[pDph_t->nphone+2])
			{
				if( pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone]-6) 
		...
		!(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FNASAL) )
	{
		if(pKsd_t->lang_curr == LANG_french)
		{
			...
			PH_DEBUG_2080_P1("Previous phone is a nasal in french -  close velum, area_n = %d\n", pDph_t->area_n);
		}
		...
		{	
			if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FSONOR)
			{
				...
				{
					if( pDph_t->phonestep && 
						pDph_t->tcum +(pDph_t->nasal_step-1) >= ((pDph_t->allodurs[pDph_t->nphone]*3)>>2) )
					{
						pDph_t->nasal_step--;
					}					
					// move faster for short phonemes
					if( pDph_t->phonestep && 
						pDph_t->tcum +(pDph_t->nasal_step-1) >= ((pDph_t->allodurs[pDph_t->nphone]*3)>>2) )
					{
						pDph_t->nasal_step--;
					}
					// make sure it is closed at the end of the phoneme.
					if (pDph_t->tcum==pDph_t->allodurs[pDph_t->nphone]-1)
					{
						pDph_t->nasal_step=0;
					}
				...
				{
					// In English This does NOT belong here.  In a sonorant between 2 nasals, the entire sonorant is nasalized 
					// 
					//pDph_t->nasal_step--;
				}

#ifdef TESTING_NOT_YET
				if (!(pDph_t->allofeats[pDph_t->nphone] & FSTRESS))
				{
					// drop faster is unstressed
					if( pDph_t->nasal_step > 2 )
						pDph_t->nasal_step--;
				}
#endif
				...
				PH_DEBUG_2080_P1("Previous phone was a nasal - close velum, area_n = %d\n", pDph_t->area_n);
					...
					pDph_t->nphone < (pDph_t->nallotot-2))
				{
					...
					PH_DEBUG_2080("Injected silence nstep to 0 \n");

				}
			}
			else	//  Nasal -> non-Sonorant so shut the velum quicker 		
			{
				...
				if ((phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FOBST) &&
					...
					(phone_feature_2(pDph_t,pDph_t->nphone) & FVOICD))
				{
					//shut velum slower for a voicebar
					if( pDph_t->nasal_step >0 && 
						pDph_t->tcum+pDph_t->nasal_step>=pDph_t->allodurs[pDph_t->nphone]-2)
						pDph_t->nasal_step -= 1;

					// shut it faster if the next phoneme is short...
					if( pDph_t->nasal_step > 0 && 
						pDph_t->tcum+pDph_t->nasal_step>=pDph_t->allodurs[pDph_t->nphone]-2)
						pDph_t->nasal_step -= 1;
					...
					if (pDph_t->nasal_step <= 1)
					{
						if (pKsd_t->lang_curr==LANG_spanish || pKsd_t->lang_curr==LANG_latin_american)
						{
							pDph_t->target_ag=600; // stronger for spanish...
						}
						else
						{
					...
					PH_DEBUG_2080_P2(" Nasal -> Voiced Stop - close velum, area_n = %d target_ag to %d\n",
						pDph_t->area_n, pDph_t->target_ag);
				}	// Nasal -> voiced stop
				else
				{
				...
				PH_DEBUG_2080_P1("Previous phone is a nasal-drop velum,area_n = %d\n",pDph_t->area_n);
