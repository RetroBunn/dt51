...
void Do_ue_dc(LPTTS_HANDLE_T phTTS)
{
		...
		)
	{
			...
			&& !(phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone)]) & FNASAL))
		{
			...
			{
				if (pDph_t->allodurs[pDph_t->nphone]<4)
				{
			...
			pDph_t->allophons[ pDph_t->nphone] == GEN_SIL )
		{
				...
				&& phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone)]& FOBST))))
			{
				...
				if( ((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FBURST) &&		// Last 3 frames of a burst
					 ...
					 pKsd_t->lang_curr != LANG_latin_american )
					|| 
					((phone_feature(pDph_t, pDph_t->allophons[pDph_t->nphone]) & FSON1) &&		// First 6 frames of a FSON1
					  pDph_t->phonestep < 6) )
				{
					if(pDph_t->dcstep > 1)
					{
...
//					pDph_t->area_ap+=100;
					...
					PH_DEBUG_2080_P2("Voiced obstruent after nasal so move glottis to %d and ap to %d faster to avoid strong voicing\n",
...
void Do_Nasal(LPTTS_HANDLE_T phTTS)
{
	...
	if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FNASAL )
	{
			...
			&& (pDph_t->allodurs[pDph_t->nphone] - pDph_t->tcum) <= 3   )				// Near end of nasal
		{
			...
			PH_DEBUG_2080(" Nasal --> voiced obst non burst so shut velum early\n");
		}
