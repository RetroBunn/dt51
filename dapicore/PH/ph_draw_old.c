				 ...
				 )
		{
			...
			if(pDph_t->target_ag >= pVtm_t->NOM_VOIC_GLOT_AREA+ 800)
			{
				if( pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone]-NF15MS) &&				// within 2 frames of the end of this phoneme
					(phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone+1]) & FVOICD))		// next phoneme is voiced
				{
...
#ifndef ODED 			
#ifndef TOMBUCHLER 
			if(  (pDph_t->nphone  >= pDph_t->last_sonorant /*|| pDph_t->nphone>(pDph_t->nallotot -4)*/)
