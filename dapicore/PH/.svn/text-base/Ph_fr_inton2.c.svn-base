...
void make_f0_command (LPTTS_HANDLE_T phTTS,short type, const int rulenumber, 
...
#define NB_PH_POS 6  // Number of values in f0_mphrase_position
  ...
  int struccur  = 0;   // current phone structure features (allofeat)
  int strucnex  = 0;   // next phone structure features (FMOT, FSYNT,etc.))
  ...
  short Syll_Mot_Restantes=0;  // Nb of syllables left in the word
  short Syll_Synt_Restantes=0; // Nb of syllables left in the clause (syntagme)
  ...
  //pDph_t->Syll_Synt           = (pDph_t->allofeats [0] & 0xFFFF) >> 8;
  // STORING NUMBER SYLS IN FEATURE RUNS OUT OF FEATURE BITS FOR THE ALL LANGUAGE MODEL
  //ll_Synt_Restantes = pDph_t->Syll_Synt;

	Syll_Synt_Restantes =	pDph_t->NbSyllabes;
  ...
  for (nphon = 0; nphon < pDph_t->nallotot; nphon++) {
    ...
    if (struccur & FMOT) {  // word or clause boundary
		//oui non rule
		if(pDph_t->allophons [nphon] == FP_W
			&& pDph_t->allophons [nphon+1] == FP_I
			&& pDph_t->allofeats[nphon+2] & FMOT)
		
 		 make_f0_command (phTTS, IMPULSE, 2, 160, 2, 20, &cumdur, nphon);
			if(pDph_t->allophons [nphon] == FP_N
			&& pDph_t->allophons [nphon+1] == FP_ON
			&& pDph_t->allofeats[nphon+2] & FMOT)
		make_f0_command (phTTS, IMPULSE, 2, 180, 2, 20, &cumdur, nphon);
    ...
    if ((pDph_t->f0mode == PHONE_TARGETS_SPECIFIED) ||
      ...
      if ( (feacur & FSYLL) && Mot_Accentue) {
        ...
        if (pDph_t->f0mode == HAT_F0_SIZES_SPECIFIED) {
          ...
          targf0 = (short)muldv (pDph_t->arg1, targf0, 32);
