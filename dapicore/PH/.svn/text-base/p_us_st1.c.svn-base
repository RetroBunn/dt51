...
// if we're at beggining silence then sil value is the value of the next 
	...
	else if (pDphsettar->par_type IS_AV_OR_AH)
	{
		...
		if (npar == AV - 1)
		{
			...
			/* Voiced fricative is devoiced if next segment unvoiced.  
				Change to light voicing */
...
*/
		...
		// blocked this shortening rule for affricates.
		if ((feanex & (FNASAL )) IS_PLUS &&
			!((feacur & FBURST) &&! (feacur & FPLOSV)))
	...
	/* Don't release burst in homorganic plosiv-nasal & plos-plos sequence */
		...
		if ((pDphsettar->phonex  == USP_LX) || (pDphsettar->phonex  == USP_LL))
		{
			bdur +=1;
