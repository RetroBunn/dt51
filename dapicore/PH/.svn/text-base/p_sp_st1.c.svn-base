			...
			&& pDphsettar->phcur != SPP_Z)
		{
				...
				|| (pDphsettar->phcur == SPP_GH))
			{
				if ((feacur & FPLOSV) IS_PLUS)
				{
					if ((feacur & FVOICD) IS_PLUS)
					{
						pDphsettar->bouval = 3;	//pDphsettar->np->tarend - 3;
						pDphsettar->durtran = NF45MS;
 ...
 *      Function Name:       
 *
 *  	Description: special_rules - called from ph_setar.c::phsettar()
							  ...
							  short struclm2, short struccur, short pholas, short struclas)
{
	...
	}	// Rule 2: Voice onset time for aspirated plosives
	...
	/***********************************************************************
		...
		Set F1 to under 250 to signal voicebar
		...
		(feacur & FBURST) IS_PLUS )
	{
		...
		PAV.pspesh = 35;	// Amp of voicebar
		PB1.pspesh = 90;	// 500;
		PB2.pspesh = 1000;
		PB3.pspesh = 1500;
		PF1.pspesh = 220;	// keep f1 low during closure
	}
}
