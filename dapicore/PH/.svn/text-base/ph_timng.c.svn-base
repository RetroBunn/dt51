...
void init_timing(LPTTS_HANDLE_T phTTS)
{
	...
	if (pKsd_t->sprate != pDphsettar->sprlast)
	{
		...
		{	
			...
			phTTS->lts_funcs[phTTS->cur_lts_lang]->Mand_F0_machine(phTTS,MFZ_INIT,NULL,MAXMANDF0SYLLABLES, MAXMANDF0INTERVALS ,pKsd_t->sprate);
#else
			Mand_F0_machine(phTTS,MFZ_INIT,NULL,MAXMANDF0SYLLABLES, MAXMANDF0INTERVALS ,pKsd_t->sprate);
#endif
		}
		...
		*/
		...
		pDphsettar->sprat1 = (short)muldv(FRAC_ONE, temp2, temp3);
		...
		if (pDphsettar->sprat0 > 180)
		{
			...
			if (temp2 <= 0)
				temp2 = 1;
			pDphsettar->sprat2 = (short)muldv(FRAC_ONE, temp2, temp3);
		}
		else
		{
...
/*FUNCTION_HEADER**********************
 * NAME:	;min_timing
 * DESC: 	Get the minimum duration value for a phoneme
 * IN:		phTS - TTS struct
			phone - phoneme value
 * OUT:		
 * RETURN:	
...
/*FUNCTION_HEADER**********************
 * NAME:	;inh_timing
 * DESC: 	Get the inherent timing of a phoneme.  These values are set in PHEDIT
 * IN:		phTTS - TTS struct
			phone - phoneme value
 * OUT:		
 * RETURN:	
