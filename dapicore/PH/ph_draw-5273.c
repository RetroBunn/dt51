...
extern char *phprint (short c);
	...
	const short nasalization[13]={0, 0, 50, 80, 120, 180, 200,260,320,450,675,780,885};
			...
			*/
			...
			if(np == &PAV)
			{
				...
				if(np->tspesh)
				{
					...
					if(/*pKsd_t->lang_curr == LANG_spanish ||*/ pKsd_t->lang_curr == LANG_french)
					{
					...
					if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FSON1)
					{
						...
						else if(np->tspesh <= 5)
						{
							...
							if(pDph_t->area_g > pVtm_t->NOM_VOIC_GLOT_AREA+900)
								pDph_t->agspeed = 1;
							else
								pDph_t->agspeed =pVtm_t->VOT_speed;
						}
	...
	*parp += (pDph_t->spdeftltoff - 3); /*from fphdraw.c*/
							...
							//	if(!(place(pDph_t->allophons[pDph_t->nphone+1]) & BLADEAFFECTED))
								{
									...
									pDph_t->tbstep = -1;
									pDph_t->target_tb = pVtm_t->NOM_Fricative_Opening+60;
								}
							}
	...
	if (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FOBST)
		...
		{	
			...
			if(pDph_t->tcum >= (pDph_t->allodurs[pDph_t->nphone]-2))
			{
				pDph_t->target_ag = pVtm_t->NOM_VOIC_GLOT_AREA+1400;
				pDph_t->agspeed = 2;
		...
		if(pDph_t->allophons[pDph_t->nphone] == UKP_HX )
				...
				&& phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FCONSON)
			{
				...
				//move it quickly to void voicing if transitioning
				// not clean fix up when more time for analysis
				if(pDph_t->area_g < pVtm_t->NOM_VOIC_GLOT_AREA+1000)
...
//#ifdef using_other more general	
		...
		&& phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone+1]) & FOBST )
	{
		if(phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone+1]) & FVOICD)
		{
				...
				&& pDph_t->tcum >= pDph_t->allodurs[pDph_t->nphone]>>1)
			{
				...
				if(DT_DBG(PH_DBG,0x080))
				{
				FNX_PRINTF(" Anticipating voiced obstruent spread glottis  \n");
				}
#endif
		...
		
	{
		...
		if(pDph_t->target_ag >= pVtm_t->NOM_VOIC_GLOT_AREA+ 800)
		{
				...
				&& (phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone+1]) & FVOICD))
			{
				...
				if(DT_DBG(PH_DBG,0x080))
				{
					FNX_PRINTF(" Anticipating voicing use high agspeed set  glottis target  %d \n", pDph_t->target_ag);
				}
#endif
...
{		
		...
		&& pDph_t->allophons[pDph_t->nphone] != GEN_SIL)
	{
		...
		{	//plosive release sits stays at fric value until one step after bstep =0
			...
			if(DT_DBG(PH_DBG,0x080))
			{
				FNX_PRINTF(" Phone with burst with body_affected so  tb=areea %d\n",	pDph_t->area_tb);
			}
#endif
		}
		else
...
//					}
					...
					{	
						...
						{	
							...
							{	//plosive release sits stays at fric value until one step after bstep =0
								...
								if(DT_DBG(PH_DBG,0x080))
								{
									FNX_PRINTF(" Phone with burst with lips_affected so target l=areea %d\n",
										pDph_t->area_l);
								}
#endif
							}
							else
							{
								if((place(pDph_t->allophons[pDph_t->nphone])) & FLABIAL )
									...
									if(DT_DBG(PH_DBG,0x080))
									{
										FNX_PRINTF(" Affricate release with lips so target_l to pVtm_t->NOM_fric \n");
									}
#endif
									pDph_t->area_l = pVtm_t->NOM_Fricative_Opening;
#ifdef PH_DElUG
		...
		if(place(pDph_t->allophons[pDph_t->nphone-1]) & BLADEAFFECTED 
			...
			if(DT_DBG(PH_DBG,0x080))
			{
				FNX_PRINTF("COARTICULATING THE BALDE  \n");
			}
#endif		
...

	{
			...
			{
			if( (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1]) & FPLOSV) IS_PLUS)
				tmp = 4;
			else
				tmp=2;
			if( pDph_t->allophons[pDph_t->nphone] == USP_TH
			|| pDph_t->allophons[pDph_t->nphone] == UKP_TH)
					tmp = pDph_t->allodurs[pDph_t->nphone]-3;
			...
			if (pDph_t->phonestep <  (pDph_t->allodurs[pDph_t->nphone]-tmp))
				...
				//	if((pDph_t->allofeats[pDph_t->nphone] & FBOUNDARY) >= FWBNEXT)
					...
					if(pDph_t->target_b  < pVtm_t->NOM_Fricative_Opening )
						pDph_t->target_b +=16;
				}	
				}
	else
	...
	if(phone_feature(pDph_t,pDph_t->allophons[ pDph_t->nphone]) & FOBST 
		...
		if( pDph_t->tcum >= pDph_t->allodurs[pDph_t->nphone]-18)
				{
					
					if(pDph_t->target_b >= 150)
						pDph_t->target_b -= 50;
					
					pDph_t->target_ap= 300;
...
//				&& (pDph_t->nallotot - pDph_t->nphone) <= 10)
			...
			if(  pDph_t->nphone  >= pDph_t->last_sonorant)
			{
				if(pDph_t->delta_area_g < 1900)
				{
					...
					if(DT_DBG(PH_DBG,0x080))
					{
						FNX_PRINTF(" Spreading  glotis at final sonorant delta_g value %d and pressure_droping also %d target %d\n",pDph_t->area_g,pDph_t->pressure_drop,pDph_t->target_ag);
					...
					&& pDph_t->syl_pressure == 0)
				{
						...
						&& pDph_t->nphone+2 <= pDph_t->nallotot )
					{
							...
							pDphsettar->nframb > 9 )
						{
							...
							{
								//research by slifka 
								if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->last_real_phon ]) & FVOICD)	// last phoneme is a voiced obstruent
								{
										if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->last_real_phon ]) & (FOBST+FCONSON)
											...
											pDph_t->syl_pressure = (((pDph_t->pressure) - 100)/(value));
								}
									else
								{
										if(phone_feature(pDph_t, pDph_t->allophons[pDph_t->last_real_phon ]) & FSTOP)
											//plosive release
										pDph_t->syl_pressure = (((pDph_t->pressure) -370)/(value));
									else
											pDph_t->syl_pressure = (((pDph_t->pressure) -550)/(value));
								}
							}
							else	// !FOBST
							{
					...
					{
						//if(pDph_t->syl_pressure > 0)
						FNX_PRINTF("Pressure drop from last syl ste to per frame %d  \n",pDph_t->syl_pressure);
					}
#endif
	...
	//	pDph_t->pressure_drop += pDph_t->syl_pressure;
							...
							&& (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FSYLL))
						{
							if(!(pDph_t->allofeats[pDph_t->nphone ] & FDUMMY_VOWEL))
							{
									...
									//the if is needed to make it wait for end of vot
								{
									...
									if(DT_DBG(PH_DBG,0x080))
									{
										FNX_PRINTF("spread glottis for unstressed phonemes  \n");
									}
#endif
...
#ifdef HLSYN		
				if( pDph_t->parstochip[OUT_PS] <= pVtm_t->NOM_Sub_Pressure -400 && pDph_t->nphone > 1
					)
				{
		...
		if(pKsd_t->sprate >= 350)
			// ADD IN FOR HIGH SPEED 
				...
				{
					if(pDph_t->target_ag == 0 && ((pDph_t->target_ag - pDph_t->area_g)>>pDph_t->agspeed) > -25)
							pDph_t->area_g = 0;
...
//			FNX_PRINTF("%d \n",pDph_t->parstochip[OUT_CNK]);
			...
			//	FNX_PRINTF("%d %d %d %d \n ",tmp, pDph_t->area_g, pDph_t->_area_g,pDph_t->nphone );
...
//				short tmp1;
	 ...
	 if ( pDph_t->nphone == pDph_t->nphonetot-1)
				...
				//	pDph_t->parstochip[OUT_FNP] = 500;
					...
					if( pDph_t->lasta2 > 500 && pDph_t->phonestep <= 3)
					{
	...
	if ( pDph_t->allophons[ pDph_t->nphone] == SPP_J)
	{
		...
		r_modulation (phTTS,pDph_t->param[A2].outp, N10PRCNT, TRUE);
					...
					if (pDph_t->allophons[ pDph_t->nphone] == GRP_KH)
					{
						...
						r_modulation (phTTS,pDph_t->param[OUT_ATB].outp, N50PRCNT, TRUE);
										...
										else if ( pDph_t->allophons[ pDph_t->nphone] == JP_R)
					{
						if(pDph_t->modulcount <=7)
						{
							...
							if(!(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone-1]) & FVOICD))
							{
								...
								{
									if(pDph_t->target_ag > pVtm_t->NOM_VOIC_GLOT_AREA+100)
										pDph_t->target_ag -= 150;
									
									
									else
										pDph_t->target_ag = pVtm_t->NOM_VOIC_GLOT_AREA+150;
								}
							...
							//pDph_t->parstochip[OUT_AG] = 900							//tr blend is a hack at the moment
							pDph_t->parstochip[OUT_A2] = 2100;
							if (pKsd_t->lang_curr == LANG_japanese)
							{
		...
		|| pDph_t->allophons[ pDph_t->nphone] == ITP_R)
					{
						if(pDph_t->modulcount <=7)
						{
							...
							if (pKsd_t->lang_curr == LANG_japanese)
							{
								pDph_t->parstochip[OUT_ABLADE] = 900;
								rs_modulation (phTTS,pDph_t->param[AREAB].outp, N97PRCNT, TRUE);
							}
	...
	if(pDph_t->nphone != pDph_t->nphonelast)
	{
			...
			
		{
			...
			void	Do_ue_dc(LPTTS_HANDLE_T phTTS)
			{
				...
				{
				if( pDph_t->tcum >= pDph_t->allodurs[pDph_t->nphone]-10
					...
					)
				{
					...
					else 
					{
						...
						if(phone_feature( pDph_t, pDph_t->allophons[( pDph_t->nphone)]) & FOBST
							&& pDph_t->area_n ==0 
							&& pDph_t->tcum <= pDph_t->allodurs[pDph_t->nphone]-NF60MS) 
						{
							//if dcstep is of opposite sign it means we're obstr-obstru so chnage faste
							if(pDph_t->dcstep > 0)
							{
...
void	Set_pressure(LPTTS_HANDLE_T phTTS)
{
	...
	{		
		...
		if(pDph_t->tcum <= pDph_t->allodurs[pDph_t->nphone]-NF130MS)
		{
			if(pDph_t->stress_pulse > 0)
				pDph_t->stress_pulse -=  pVtm_t->STRESS_STEP+30;
		}
		else
		{
			if(pDph_t->stress_pulse < pVtm_t->STRESS_PRESSURE+200)
				pDph_t->stress_pulse += pVtm_t->STRESS_STEP+30;
		}
	}
	...
	{		
		...
		else
		{
			...
			{
				FNX_PRINTF("stressed syl build pressure-stress pulse %d\n",pDph_t->stress_pulse);
			}
#endif
	...
	if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FOBST)
	{
		...
		if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & 004000)
		{
			...
			{	
				...
				if(DT_DBG(PH_DBG,0x080))
				{
					FNX_PRINTF("the lips so maybe the blade\n");
				}
#endif
				
				//labial plosive obst
				// and force rapid closure
				//pDph_t->area_l = 200;
				pDph_t->target_l= 0;
				pDph_t->target_b = 1000;
				pDph_t->target_tb=1000;
		...
		else 
		{
			...
			if(DT_DBG(PH_DBG,0x080))
			{
				FNX_PRINTF("SPANISH velar approximate ...");
			}
#endif
			...
			else if((place(pDph_t->allophons[pDph_t->nphone])) & FGLOTTAL )
			{
				...
				if(DT_DBG(PH_DBG,0x080))
				{
					FNX_PRINTF("the glottis to close\n");
				}
#endif	
				pDph_t->target_l= 1000;
				pDph_t->target_tb= 1000;
		...
		if(phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone]) & FNASAL )
		{
				...
				&& place(pDph_t->allophons[pDph_t->nphone-1]) &  BLADEAFFECTED)
			{
				...
				if(pDph_t->nasal_step ==0)
				 ...
				 && (phone_feature(pDph_t,pDph_t->allophons[pDph_t->nphone+1]) & FSTOP )
				 && (pDph_t->allodurs[pDph_t->nphone] - pDph_t->tcum) <= 6 )				// Near end of nasal
			{
				...
				if(DT_DBG(PH_DBG,0x080))
				{
					FNX_PRINTF("phone a nasal-- next unvoiced obst stop so shut velum early \n");
				}
