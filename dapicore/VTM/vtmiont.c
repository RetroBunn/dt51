...
int vtm_loop(LPTTS_HANDLE_T phTTS,unsigned short *input)
{
	...
	
	{
		...
		// Keep track of the current frame so that we can output some timing information
		// if :debug 2040 is requested.
		if( pDph_t->nphone == 0 &&
			pDph_t->tcum == 0 )
			pVtm_t->frame.n = 0;
		...
		case SPC_type_voice:
			...
			if (( ! pKsd_t->halting ) && ( ! phTTS->bMemoryReset ))
			{
...
#endif //hlsyn
				// Keep track of the number of frames so we can print time information for debugging
				pVtm_t->frame.n++;
				...
				{
					static char sLastPhone[12];
					static int nSinceLastHeader=0;
					if( FNX_STRCMP(sLastPhone, phprint(pVtm_t->parambuff[OUT_PH+1])) != 0 )
					{
						FNX_STRCPY(sLastPhone, phprint(pVtm_t->parambuff[OUT_PH+1]));
						nSinceLastHeader = 0;
						FNX_PRINTF("\n=================================== %5s ============================================= %5s ================================= %5s\n", 
							sLastPhone, sLastPhone, sLastPhone);
					}
					if( nSinceLastHeader == 0 || 
						nSinceLastHeader % 10 == 0 )
					{
						FNX_PRINTF("\n\n%5s %4s %4s %4s | %3s %4s %4s  | %4s %4s %4s %4s %6s %6s %7s %7s |  %2s %2s %2s  %2s  %2s  %2s  %2s  %2s  %2s  %2s\n",
							"Time", "B1", "AG", "A2", "F1", "F2", "F3",
							"AL", "ATB", "ABLD", "AN", "CHNK", "PS", "UE", "DC",
							"AF", "AH", "AV", "A2", "A3", "A4", "A5", "A6", "AB", "F0");
						nSinceLastHeader++;
					}
					else
					{
					...
					FNX_PRINTF("%04.3f %4.4d %4.4d %4.4d | %3.3d %4.4d  %4.4d | %4.4d %4.4d %4.4d %4.4d % 6.2f % 6.2f % 07.2f % 07.2f | ",
						(float)pVtm_t->frame.n*(float)pVtm_t->uiNumberOfSamplesPerFrame/(float)pVtm_t->SampleRate,
						pVtm_t->parambuff[OUT_B1+1],
						pVtm_t->parambuff[OUT_AG+1],
						pVtm_t->parambuff[OUT_A2+1],
						pVtm_t->parambuff[OUT_F1+1],
						pVtm_t->parambuff[OUT_F2+1],
						pVtm_t->parambuff[OUT_F3+1],
						pVtm_t->parambuff[OUT_AL+1],
						pVtm_t->parambuff[OUT_ATB+1],
						pVtm_t->parambuff[OUT_ABLADE+1],
						pVtm_t->parambuff[OUT_AN+1],
						pVtm_t->frame.ap,
						pVtm_t->frame.ps,
						pVtm_t->frame.ue,
						pVtm_t->frame.dc);
				...
				if (pVtm_t->last_OUT_A2!=pVtm_t->parambuff[OUT_A2+1])
					pVtm_t->oldA2=0;

				pVtm_t->last_OUT_A2=pVtm_t->parambuff[OUT_A2+1];
				...
				if(pVtm_t->parambuff[OUT_A2+1] >= 2000 
					...
					pVtm_t->in_region =0;
				if(pVtm_t->parambuff[OUT_A2+1]  > 100)	
				{
					tmp = pVtm_t->parambuff[OUT_A2+1];
				...
				else if(pVtm_t->parambuff[OUT_A2+1] == 1100)
				{
					...
					pVtm_t->llframe.NA5F = 0;
					pVtm_t->llframe.NA6F = 0;
					pVtm_t->llframe.NAB = 63;
					
				}
				else if(pVtm_t->parambuff[OUT_A2+1] == 2201)
				{
					//korean ch and cc
					pVtm_t->llframe.NA2F = 21+ (pVtm_t->oldA2);
					pVtm_t->llframe.NA3F =35 + (pVtm_t->oldA2);
					pVtm_t->llframe.NA4F = 62- (pVtm_t->oldA2>>1);
					pVtm_t->llframe.NA5F = 42;
					pVtm_t->llframe.NA6F = 45;
					//high compute put in low compute
					if(pVtm_t->oldA2== 0 )
					{
						pVtm_t->llframe.NA2F = 0;
						pVtm_t->llframe.NA3F += 5;
						pVtm_t->llframe.NA4F += 5;
						pVtm_t->llframe.NA5F += 5;
						pVtm_t->llframe.NA6F = 0;
					}
					
					if( pVtm_t->llframe.NAF > 20)
					{
						if(pVtm_t->oldA2 < 25)
							pVtm_t->oldA2 += 2;
	...
	else if(pVtm_t->parambuff[OUT_A2+1] == 1200)
	{
		...
		pVtm_t->llframe.NA2F = 20;
		pVtm_t->llframe.NA3F = 10;
		pVtm_t->llframe.NA4F = 10;
		pVtm_t->llframe.NA5F = 10;
		pVtm_t->llframe.NA6F = 35;
		pVtm_t->llframe.NAB =  56;
	}
	else if(pVtm_t->parambuff[OUT_A2+1] == 1201)
	{
		//th  weak burst
		pVtm_t->llframe.NA2F = 0;
		pVtm_t->llframe.NA3F = 5+(pVtm_t->oldA2>>1);
		pVtm_t->llframe.NA4F = 5+(pVtm_t->oldA2);
		pVtm_t->llframe.NA5F = 8+(pVtm_t->oldA2);
		pVtm_t->llframe.NA6F = 0;
		pVtm_t->llframe.NAB =  53;
		
#ifdef HLSYN
		if( pVtm_t->llframe.NAF > 20)				// High Compute
#else
			if( pVtm_t->parambuff[OUT_GF+1] > 20)		// Low Compute {}
#endif
			{
				if(pVtm_t->oldA2 < 25)
					pVtm_t->oldA2 += 3;
			...
			else
			{
				...
				else if(pVtm_t->parambuff[OUT_A2+1] == 1301)
				{
					// Japanese F
					pVtm_t->llframe.NA2F = 38;
					pVtm_t->llframe.NA3F = 30;
					pVtm_t->llframe.NA4F = 25;
					pVtm_t->llframe.NA5F = 25;
					pVtm_t->llframe.NA6F = 0;
					pVtm_t->llframe.NAB = 0;
				}
				else if(pVtm_t->parambuff[OUT_A2+1] == 2123)
				{
					//retro-flexed
					if (pVtm_t->f2_ref > 2050)
					{
						pVtm_t->llframe.NA2F = 45;
						pVtm_t->llframe.NA3F = 55;
						pVtm_t->llframe.NA4F = 0;
						pVtm_t->llframe.NA5F = 0;
						pVtm_t->llframe.NA6F = 0;
						pVtm_t->llframe.NAB = 0;
				...
				else if(pVtm_t->parambuff[OUT_A2+1] == 2001)
				{
					pVtm_t->llframe.NA2F = 30;
					pVtm_t->llframe.NA3F = 40+pVtm_t->oldA2;
					pVtm_t->llframe.NA4F = 45+pVtm_t->oldA2;
					pVtm_t->llframe.NA5F = 10;
					pVtm_t->llframe.NA6F = 5;
					pVtm_t->llframe.NAB = 0;
#ifdef HLSYN
					if( pVtm_t->llframe.NAF > 20)				// High Compute
#else
				...
				else if(pVtm_t->parambuff[OUT_A2+1] == 2000)
				{
					...
					else
					{
						...
						{
							pVtm_t->llframe.NA2F = 0;
							pVtm_t->llframe.NA3F = 47;
							pVtm_t->llframe.NA4F = 36;
							pVtm_t->llframe.NA5F = 15;
							pVtm_t->llframe.NA6F = 5;
							pVtm_t->llframe.NAB = 0;
						}
					}
				...
				else if(pVtm_t->parambuff[OUT_A2+1] == 2500)
				{
					//PALATEL-alvelar
					pVtm_t->llframe.NA2F = 0;
					pVtm_t->llframe.NA3F = 25+(pVtm_t->oldA2);
				...
				{
					//ch roll
					//tm_t->llframe.NF3 += (160-(pVtm_t->oldA2<<1));
					pVtm_t->llframe.NA2F = 0;
					pVtm_t->llframe.NA3F = 25+(pVtm_t->oldA2);
					pVtm_t->llframe.NA4F = 35+(pVtm_t->oldA2>>1);
...
#endif
					{
						...
						if(pVtm_t->oldA2 ==6)
						{
							pVtm_t->llframe.NA3F -=18;	
							pVtm_t->llframe.NA4F -=25;
						}
						if (pVtm_t->oldA2<6)
						{
							pVtm_t->llframe.NA3F +=15;
	...
	else if(pVtm_t->parambuff[OUT_A2+1] == 2201)
	{
		...
		else 
		{
			...
			pVtm_t->llframe.NAB = 0;
				...
				if(pVtm_t->parambuff[OUT_A2+1] == 5000)		// FVELAR
				{			
					...
					{
						// this needs changing.  for kw only a2, for kl only a5
						pVtm_t->llframe.NA2F = 50;
						pVtm_t->llframe.NA3F = 0;
						pVtm_t->llframe.NA4F = 0;
						pVtm_t->llframe.NA5F = 40;
					...
					else if(pVtm_t->parambuff[OUT_PLACE+1] == 80)	// GRP_KH, SPP_J, LAP_J
					{			  
						pVtm_t->llframe.NA2F = 46;
						pVtm_t->llframe.NA3F =  0;
						pVtm_t->llframe.NA4F = 20;
						pVtm_t->llframe.NA5F =  0; 
						pVtm_t->llframe.NAB =0;
				...
				else if(pVtm_t->parambuff[OUT_A2+1] == 5001)	// JP_I
				{			
					if(pVtm_t->parambuff[OUT_PLACE+1] == 40)
					{
						pVtm_t->llframe.NA2F = 35;
						pVtm_t->llframe.NA3F = 58;
						pVtm_t->llframe.NA4F = 50;
						pVtm_t->llframe.NA5F = 0; 
						pVtm_t->llframe.NAB =0;
					}
				...
				//For liquids  
				else if(pVtm_t->parambuff[OUT_A2+1] == 4000)
				{
...
#ifdef TESTING_THIS_CODE
	temp3=(pVtm_t->llframe.NA2F +pVtm_t->llframe.NA3F +pVtm_t->llframe.NA4F+
		pVtm_t->llframe.NA5F+pVtm_t->llframe.NA6F+pVtm_t->llframe.NAB);
	temp4=pVtm_t->last_a2+pVtm_t->last_a3+pVtm_t->last_a4+pVtm_t->last_a5+
		pVtm_t->last_a6+pVtm_t->last_ab;

	if (  pVtm_t->llframe.NAB < (pVtm_t->last_ab-40) ||
		  pVtm_t->llframe.NA2F < (pVtm_t->last_a2-40) ||
		  pVtm_t->llframe.NA3F < (pVtm_t->last_a3-40) ||
		  pVtm_t->llframe.NA4F < (pVtm_t->last_a4-40) ||
		  pVtm_t->llframe.NA5F < (pVtm_t->last_a5-40) ||
		  pVtm_t->llframe.NA6F < (pVtm_t->last_a6-40) )
	{	
		pVtm_t->parambuff[OUT_A2+1] = (pVtm_t->last_a2+pVtm_t->llframe.NA2F)>>1;
		pVtm_t->parambuff[OUT_A3+1] = (pVtm_t->last_a3+pVtm_t->llframe.NA3F)>>1;
		pVtm_t->parambuff[OUT_A4+1] = (pVtm_t->last_a4+pVtm_t->llframe.NA4F)>>1;
		pVtm_t->parambuff[OUT_A5+1] = (pVtm_t->last_a5+pVtm_t->llframe.NA5F)>>1;
		pVtm_t->parambuff[OUT_A6+1] = (pVtm_t->last_a6+pVtm_t->llframe.NA6F)>>1;
		pVtm_t->parambuff[OUT_AB+1] = (pVtm_t->last_ab+pVtm_t->llframe.NAB)>>1;
	...
	pVtm_t->last_a2=pVtm_t->llframe.NA2F;
	pVtm_t->last_a3=pVtm_t->llframe.NA3F;
	pVtm_t->last_a4=pVtm_t->llframe.NA4F;
	pVtm_t->last_a5=pVtm_t->llframe.NA5F;
	pVtm_t->last_a6=pVtm_t->llframe.NA6F;
	pVtm_t->last_ab=pVtm_t->llframe.NAB;
			
	pVtm_t->llframe.NA2F=pVtm_t->parambuff[OUT_A2+1];
	pVtm_t->llframe.NA3F=pVtm_t->parambuff[OUT_A3+1];
	pVtm_t->llframe.NA4F=pVtm_t->parambuff[OUT_A4+1];
	pVtm_t->llframe.NA5F=pVtm_t->parambuff[OUT_A5+1];
	pVtm_t->llframe.NA6F=pVtm_t->parambuff[OUT_A6+1];					
	pVtm_t->llframe.NAB=pVtm_t->parambuff[OUT_AB+1];
#endif
					...
					{
#ifdef INTEGER
						FNX_PRINTF("%02d  %02d  %02d  %02d  %02d  %02d  %02d  %02d  %02d  AP %02d T0 %02d\n",
							pVtm_t->parambuff[OUT_GF+1],	// Gain of Frication
							pVtm_t->parambuff[OUT_AP+1],	// Aspiration
							pVtm_t->parambuff[OUT_AV+1],	// Amp of voicing
							pVtm_t->parambuff[OUT_A2+1],
							pVtm_t->parambuff[OUT_A3+1],
							pVtm_t->parambuff[OUT_A4+1],
							pVtm_t->parambuff[OUT_A5+1],
							pVtm_t->parambuff[OUT_A6+1],
							pVtm_t->parambuff[OUT_AB+1],
							pVtm_t->parambuff[OUT_AP+1],
							(400000/pVtm_t->parambuff[OUT_T0+1]));
					
#else
					
						FNX_PRINTF(" %02d %02d %02d  %02d  %02d  %02d  %02d  %02d  %02d  %02d\n",
							pVtm_t->llframe.NAF,
							pVtm_t->llframe.NAH,
							pVtm_t->llframe.NAV,
							pVtm_t->llframe.NA2F,
...
if(pKsd_t->lang_curr == LANG_french )
	{
		...
		pVtm_t->parambuff[OUT_FZ+1] = (pVtm_t->fzinlast + pVtm_t->parambuff[OUT_FZ+1])>>1;
		pVtm_t->fzinlast = pVtm_t->parambuff[OUT_FZ+1];
	}
		  
	
#ifdef HLSYN
	//	FNX_PRINTF("out_f1,  %d %f %f\n",pVtm_t->parambuff[OUT_F1+1],frame.al,frame.ag); 		  
	if(pVtm_t->llframe.NF0 > 20)
		pVtm_t->parambuff[OUT_T0+1]=  (400000 / (pVtm_t->llframe.NF0));
	else
		pVtm_t->parambuff[OUT_T0+1]= 0;
	...
	if(DT_DBG(PH_DBG,0x100))
	{
		...
		FNX_PRINTF(" F0 %d ", pVtm_t->llframe.NF0);
...
#ifdef HLSYN
		InitializeHLSynthesizer(&pVtm_t->oldframe, &pVtm_t->speakerDef.speaker, 
			&pVtm_t->oldstate, pVtm_t->parambuff[OUT_SEX+1] );
#endif
		
		changeSpeakerValues(phTTS, &pVtm_t->speakerDef, pKsd_t->last_voice, &(pVtm_t->parambuff[1]));
		break;
				...
				if (phTTS->EmbCallbackRoutine((short *) &(input[1]),3)==NULL) // index returned
					...
					if ( ! pKsd_t->halting )
					{
						...
						case (SPC_type_index|SPC_subtype_volume):
							...
							// finally, send the pointer to this struct off to 
							// be processed.
							// MGS this will cause a problem for 64-bit machines
							pVtm_t->dwSampleNumber = (LONG)(pVtm_t->pMarkData);
							break;
						default:
							// just leave it alone.
							break;
...
void changeSpeakerValues(LPTTS_HANDLE_T phTTS, TSpeakerDef *speakerDef, currentSpeaker SpeakerName, SPD_CHIP *spdeftochip)
{
	PVTM_T pVtm_t = phTTS->pVTMThreadData;
	PKSD_T pKsd_t = phTTS->pKernelShareData;
	PDPH_T pDph_t = phTTS->pPHThreadData;
...
//			speakerDef->speaker.NewtonInterpTimeStep = .01f;
			...
			speakerDef->speaker.F5 =  speaker_def_ptr->HL_F5;
			...
			pVtm_t->EndOfPhrase_Spread_target =speaker_def_ptr->endofphrase_spread_target;
			...
			pVtm_t->f2_ref_scale =                   speaker_def_ptr->f2_ref_scale;
			pVtm_t->f3_ref_scale =  				speaker_def_ptr->f3_ref_scale;
			pDphsettar->diplo_step = 					speaker_def_ptr->diplo_step;
			pDphsettar->endpoint_below_ap = 	speaker_def_ptr->endpoint_below_ap;
			pDphsettar->endpoint_mult_stress = 		speaker_def_ptr->endpoint_mult_stress;
			pDphsettar->tune_param_1 = 				speaker_def_ptr->tune_param_1; 
			pDphsettar->tune_param_2 = 				speaker_def_ptr->tune_param_2;
			pDphsettar->init_st_no_aspir = 			speaker_def_ptr->init_st_no_aspir;
			pDphsettar->str_vow_before_final = 		speaker_def_ptr->str_vow_before_final;
			speakerDef->speaker.KTL =((float)(speaker_def_ptr->KTL))/100.0;

#else

			// NEW speaker def parameters
			speakerDef->speaker.agm = ((float)(spdeftochip->agm))/100.0f;
			speakerDef->speaker.OQm = pVtm_t->NOM_Open_Quo;
			speakerDef->speaker.TLm = pVtm_t->Tiltm;

			//speakerDef->speaker.OQm = 50;		
			speakerDef->speaker.Ka = spdeftochip->Ka;			

			speakerDef->speaker.B1m = spdeftochip->HL_B1m;
			speakerDef->speaker.B2m = spdeftochip->HL_B2m;
			speakerDef->speaker.B3m = spdeftochip->HL_B3m;
			speakerDef->speaker.B4m = spdeftochip->HL_B4m;
			speakerDef->speaker.B5m = spdeftochip->HL_B5m;
			speakerDef->speaker.B2F = spdeftochip->HL_B2F;
			speakerDef->speaker.B3F = spdeftochip->HL_B3F;
			speakerDef->speaker.B4F = spdeftochip->HL_B4F;
			speakerDef->speaker.B5F = spdeftochip->HL_B5F;
			speakerDef->speaker.F5  = spdeftochip->HL_F5;
			speakerDef->speaker.F6  = spdeftochip->HL_F6;
			speakerDef->speaker.B6F = spdeftochip->HL_B6F;		

			pVtm_t->STRESS_STEP			= spdeftochip->stress_step; // step must add to total in 8 steps or less
			pVtm_t->UNSTRESS_PRESSURE	= spdeftochip->unstress_pressure;
			pVtm_t->STRESS_PRESSURE		= spdeftochip->stress_pressure;
			pVtm_t->NOM_Sub_Pressure	= spdeftochip->nom_sub_pressure;
			pVtm_t->NOM_Fricative_Opening = spdeftochip->nom_fricative_opening;
			pVtm_t->NOM_Glot_Stop_Area	= spdeftochip->nom_glot_stop_area;
			pVtm_t->VOT_speed			= spdeftochip->vot_speed;
			pVtm_t->EndOfPhrase_Spread	= spdeftochip->endofphrase_spread;
			pVtm_t->EndOfPhrase_Spread_target	= spdeftochip->endofphrase_spread_target;
			
			speakerDef->speaker.f1HiShift	= spdeftochip->HL_f1_hi_shift;
			speakerDef->speaker.acd_f1Break = spdeftochip->HL_acd_f1_break;
			pDphsettar->ob_onset_pressure	= 0;
			pVtm_t->f2_ref_scale			= spdeftochip->f2_ref_scale;
			pVtm_t->f3_ref_scale			= spdeftochip->f3_ref_scale;
			pDphsettar->diplo_step			= spdeftochip->diplo_step;
			pDphsettar->endpoint_below_ap	= spdeftochip->endpoint_below_ap;
			pDphsettar->endpoint_mult_stress = spdeftochip->endpoint_mult_stress;
			pDphsettar->tune_param_1		= spdeftochip->tune_param_1; 
			pDphsettar->tune_param_2		= spdeftochip->tune_param_2;
			pDphsettar->init_st_no_aspir	= spdeftochip->init_st_no_aspir;
			pDphsettar->str_vow_before_final = 		spdeftochip->str_vow_before_final;
			speakerDef->speaker.KTL			= ((float)(spdeftochip->KTL))/100.0f;

#if 0
	switch(SpeakerName)
	{
		case Paul:
			speakerDef->speaker.Ka = 42;					// Minimum aspiration in voice
			speakerDef->speaker.agm = 4.0;					// Modal glottal area
			speakerDef->speaker.OQm = pVtm_t->NOM_Open_Quo;	// 
			speakerDef->speaker.B1m = 70;					// B1 modal
			speakerDef->speaker.B2m=  110;
			speakerDef->speaker.B3m=  160;
			speakerDef->speaker.B4m=  180;
			speakerDef->speaker.B5m=  200;
			speakerDef->speaker.B2F = 250;					// B2 for parallel tract (?)
			speakerDef->speaker.B3F=  320;
			speakerDef->speaker.B4F=  260;
			speakerDef->speaker.B5F=  270;
			speakerDef->speaker.TLm = pVtm_t->Tiltm;
			speakerDef->speaker.F5 =  4200;					// not used from here
			speakerDef->speaker.F6  = 4800;					// high frequency, wide bandwidth used for high frequency fill-in
			speakerDef->speaker.B6F = 600;		
			pVtm_t->STRESS_STEP = 30;						// step must add to total in 8 steps or less, increment to subglottal pressure
			pVtm_t->UNSTRESS_PRESSURE = 80;					// minimum value
			pVtm_t->STRESS_PRESSURE = 200;					// maximum value
			pVtm_t->NOM_Sub_Pressure = 800;					// nominal pressure in lungs
			pVtm_t->NOM_Fricative_Opening = 100;			// nominal frication opening area (mm^2 * 100)
			pVtm_t->NOM_Glot_Stop_Area = 0;					// nominal stop opening area (0 is fully closed)
			pVtm_t->VOT_speed = 1;							// Voice Onset Time stepping speed
			pVtm_t->EndOfPhrase_Spread =10;					// 

			speakerDef->speaker.f1HiShift = 1180;
			speakerDef->speaker.acd_f1Break = 600;			
			pVtm_t->f2_ref_scale =  -80;					// see page 7 of HLSYN supplemental docs
			pVtm_t->f3_ref_scale = -100;					// scaling factors for indices into tables
			pDphsettar->diplo_step = 1;						// diplophonic pulsing or creak step size
			pDphsettar->endpoint_below_ap = 200;			// (experimental)
			pDphsettar->endpoint_mult_stress = -100;			// (experimental)
	...
	case Betty:
			...
			speakerDef->speaker.agm = 3.5;
			speakerDef->speaker.OQm = pVtm_t->NOM_Open_Quo;
			speakerDef->speaker.B1m = 80;
//			speakerDef->speaker.NewtonInterpTimeStep = .01;
			speakerDef->speaker.B2m=  98;
			speakerDef->speaker.B3m=  145;
			speakerDef->speaker.B4m=  300;
			speakerDef->speaker.B5m=  600;
			speakerDef->speaker.B2F = 250;
			speakerDef->speaker.B3F=  320;
			speakerDef->speaker.B4F=  260;
			...
			pVtm_t->f2_ref_scale = 180;
			pVtm_t->f3_ref_scale = 180;
			pDphsettar->diplo_step = 7;
			pDphsettar->ob_onset_pressure =00;
			pDphsettar->endpoint_below_ap = 450;
			pDphsettar->endpoint_mult_stress = -100;
...
//			speakerDef->speaker.NewtonInterpTimeStep = .01f;
	...
	case Rita:	/*Rita*/
		//really sue
			speakerDef->speaker.Ka = 45;
			speakerDef->speaker.agm = 7.0;
			speakerDef->speaker.OQm = pVtm_t->NOM_Open_Quo;
...
//			speakerDef->speaker.NewtonInterpTimeStep = .01f;
	...
	default: /*paul*/
			...
			pVtm_t->f2_ref_scale = 80;
			pVtm_t->f3_ref_scale = 80;
			pVtm_t->f2_ref_scale = 0;
			pVtm_t->f3_ref_scale = 0;
