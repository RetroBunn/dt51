...
#include "dectalkf.h"
#ifdef ARM7
#pragma arm section rwdata="VTM3RWDATA", rodata="VTM3RODATA", zidata="VTM3DATA"
#endif
...
// Although this table contains 161 items, only 140 are used
const int int_volume_table[161]=
{
//0,			// Moving the volume table down to increase the overall volume
//1071,
//1110,
//1149,
//1189,
...
#define HLSYN_NEWPOLE
	...
	*/
	...
	//		FNX_PRINTF("1NPER %d t0 %d avindb %d  ph %d gf %d\n", pVtm_t->nper, pVtm_t->T0,AVinDB,variabpars[OUT_PH],AVinDB,variabpars[OUT_GF]);
	...
	APlin= amptable[APinDB+5];    /*  Convert dB to linear */
	//	FNX_PRINTF("breath %d \n",abreth);
	//	FNX_PRINTF("APlin %d \n",APlin);
	r2pg = amptable[A2inDB+3];  /*  Convert dB to linear */
	r3pg = amptable[A3inDB+7];  /*  Convert dB to linear */
	r4pa = amptable[A4inDB+8];  /*  Convert dB to linear */
	r5pa = amptable[A5inDB+8];  /*  Convert dB to linear */
	r6pa = amptable[A6inDB+6];  /*  Convert dB to linear */
...
//	r3pg = frac1mul( r3pg, amptable[pVtm_t->AFcgain] );    /*  Scale A3 by spdef GF */
	...
	for ( ns = 0; ns < pVtm_t->uiNumberOfSamplesPerFrame; ns++ )
	{
		...
		for ( nsr4 = 0; nsr4 < 4; nsr4++ )
		{
			...
			pVtm_t->avlind = amptable[AVinDB ]; //pVtm_t->avlin; 
			...
			if(AVinDB == 0)
				// just rampinmg it to a small value
				pVtm_t->voice0 = pVtm_t->voice0>>ns;
			pVtm_t->voicebeforegain = pVtm_t->voice0;
			voice = frac4mul( pVtm_t->avlind, pVtm_t->voice0 );

			voice = frac4mul( voice, pVtm_t->avgain );
			//pVtm_t->iwave[ns] = voice;		// Uncomment this to output glottal wave form only
			/****************************************************************/
			...
			if((pVtm_t->lastavindb == 0 && AVinDB > 0)
				|| ( pVtm_t->nper == pVtm_t->T0 ) || (AVinDB == 0 && pVtm_t->nper > 500))
			{
				//	printf( " %d %d %d \n ",pVtm_t->nper, pVtm_t->T0   , AVinDB);
				/**************************************************************/
				...
				pVtm_t->rnpa = d2pole_cf123( pVtm_t,pKsd_t, &pVtm_t->rnpb, &pVtm_t->rnpc,FNPinHZ , BNPinHZ ,10596 );
#endif
...
#endif
#if defined HLSYN_NEWPOLE && defined HLSYN				
				setzeroabc( FZ2inHZ,BZ2inHZ, 2500, &pVtm_t->rnz2a, &pVtm_t->rnz2b, &pVtm_t->rnz2c);
		...
		//			pVtm_t->iwave[ns] = voice;  //iwve number 1
		...
		//	}
		//pVtm_t->iwave[ns] = voice;	//iwave number 2
		...
		*/
		...
		voice += frac1mul(pVtm_t->Aturb, noiseb  ); 
...
skip_cascade:
		...
		/******************************************************************/
		if( AVinDB > 10 &&				// VOICED
			pVtm_t->AFcgain > 10 )		// FRICATIVE
		{
			if (pVtm_t->voicebeforegain ==0 )
			{
				pVtm_t->voiceint = (pVtm_t->voiceint >>1);
				if(pVtm_t->NoiseMod > 0)
				{
					pVtm_t->NoiseMod -= 2;
					noise_const=0;
				}
				else if(pVtm_t->NoiseMod > -5 )
				{
					pVtm_t->NoiseMod -= 1;
					noise_const=0;
				}
			...
			{
				//		printf(" %d  in second \n", pVtm_t->voiceint);
				pVtm_t->NoiseMod = pVtm_t->voiceint>>3;
				if (pVtm_t->NoiseMod>5)
					pVtm_t->NoiseMod=5;
				pVtm_t->voiceint += 1;
			}
								
//	printf(" %d  in second \n", pVtm_t->voiceint);			
			temp = pVtm_t->AFcgain+ pVtm_t->NoiseMod;
//			printf(" %d  temp %d \n",pVtm_t->NoiseMod, temp );
			if(temp > 87)
				temp= 87;
		}
		else
		...
		r3pamp = frac1mul( r3pa, amptable[pVtm_t->AFcgain] );    /*  Scale A4 by spdef GF */
		r4pamp = frac1mul( r4pa, amptable[temp] );    /*  Scale A4 by spdef GF */
		r5pamp = frac1mul( r5pa, amptable[temp] );    /*  Scale A5 by spdef GF */
		r6pamp = frac1mul( r6pa, amptable[temp] );    /*  Scale A6 by spdef GF */

		if(pVtm_t->par_count==0)
			goto skip_parallel;

		two_pole_filter( noisef, pVtm_t->r6pd1, pVtm_t->r6pd2, r6pamp,pVtm_t->R6pb, pVtm_t->r6pc );
		...
		{
			two_pole_filter( noisef, pVtm_t->r5pd1, pVtm_t->r5pd2, r5pamp, pVtm_t->R5pb, pVtm_t->r5pc );
		}
		else
		{
			pVtm_t->r5pd1 = 0;
		...
		{
			two_pole_filter( noisef, pVtm_t->r4pd1, pVtm_t->r4pd2, r4pamp, pVtm_t->R4pb, pVtm_t->r4pc );
		}
		else
		{
			pVtm_t->r4pd1=0;
		...
		two_pole_filter( noisef, pVtm_t->r3pd1, pVtm_t->r3pd2, r3pamp, r3pb, r3pc );
		...
		two_pole_filter( noisef, pVtm_t->r2pd1, pVtm_t->r2pd2, r2pa, r2pb, r2pc );
...
skip_parallel:
		...
		*/
		...
		else
		{
			...
			pVtm_t->iwave[ns] = out;			// Normal vocal tract output
		}
