...
/* MAIN LOOP, for each output phoneme */
	...
	for (nphon = 0; nphon < pDph_t->nallotot; nphon++)
	{
		...
		prcnt = 100;//relative speed adjustment
		...
		if ((feacur & FSYLL) IS_PLUS)
		{
			...
			{
				//Noticeon the subttraction LA_WX is used instead of SPP_WX  as it does not
		...
		if ((feacur & FCONSON) IS_PLUS)
		{
			...
			if (pDph_t->allophons[nphon+1] == SPP_S
				&& phocur == SPP_K)
			{
				deldur = -5;
			...
			else if (phocur == SPP_R || phocur == SPP_RR)
			{
				 ...
				 *      certain consonants, where it is a fSPP.
