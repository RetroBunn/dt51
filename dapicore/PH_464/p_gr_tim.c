...
void gr_phtiming (LPTTS_HANDLE_T phTTS)
{
	...
	for (nphon = 0; nphon < pDph_t->nallotot; nphon++)
	{
		...
		{
			pDphsettar->durxx = mstofr (pDph_t->user_durs[nphon] );
#ifdef MSDBG5
			printf ("durxx = mstofr(user_durs[nphon] durxx=%d\n", pDphsettar->durxx);
#endif
		...
		/* Does rule apply ? */
			if (phocur == SPP_S
					&& pDphsettar->phonex_timing == GRP_SH)
					{
					pDphsettar->durxx = NF30MS;
					durmin = 0;
					durinh = 10;
				}
		if ((feasyllabiccur IS_PLUS)
				...
				&& (pholas != GRP_S))
			{
					...
					&& (strucboucur < FVPNEXT))
				{
					...
					if ((phocur == GRP_S) )
					{
						...
						if (pDphsettar->phonex_timing == SPP_S)
						{
							pDphsettar->durxx = NF30MS;
#ifdef MSDBG5
							printf ("durxx = NF30MS durxx=%d\n", pDphsettar->durxx);
#endif
							goto break3;
						}
