...
const short durdic[] = 
{
	...
	10,WBOUND,FP_J,FP_E,WBOUND,EOS,0,80,80,0,-1,
	10,WBOUND,FP_S,FP_E2,WBOUND,EOS,0,80,80,0,-1,
	10,WBOUND,SPP_DH,SPP_E,WBOUND,EOS,0,80,180,0,-1,
	12,WBOUND,FP_P,FP_L,FP_U,WBOUND,EOS,0,100,80,120,0,-1,
	...
	12,WBOUND,USP_N,S1,USP_EH,USP_V,EOS,0,0,0,80,0,-1, 
	12,WBOUND,S1,USP_IY,USP_V,USP_EN,EOS,0,0,142,0,0,-1, 
	14,WBOUND,USP_P,USP_R,S1,USP_AA,USP_D,EOS,0,0,0,0,144,0,-1,
	EOS
};
...
// n== 1 becuase french send it's own beginning silence. 
  for (n = 0; n < pDph_t->nsymbtot; n++) {
	...
	if(
		pDph_t->symbols[n-3] == FP_K
		&& pDph_t->symbols[n-2] == FP_E2
		&& pDph_t->symbols[n-1] == FP_L
		&& pDph_t->symbols[n] == FP_K
		&& pDph_t->symbols[n+1] == WBOUND)
		if( !(phone_feature( pDph_t,  pDph_t->symbols[n + 2]) & FVOWEL))
			insertphone (phTTS, (short)(n+1), ( FP_E));
		
		if(
		pDph_t->symbols[n-1] == FP_V
		&& pDph_t->symbols[n] == FP_OU
		&& pDph_t->symbols[n+1] == WBOUND)
		if( (phone_feature( pDph_t,  pDph_t->symbols[n + 2]) & FVOWEL))
			insertphone (phTTS, (short)(n+1), ( FP_Z));
...
//	int ntmp;
	...
	short doneit=0;
		...
		short word_start=0,word_end=0;
		pDph_t->phrasal_verb =0;
...
if(pKsd_t->lang_curr == LANG_german)
{
	...
	if (pDph_t->symbols[n-2]  >= WBOUND
		&& pDph_t->symbols[n-1]  == S1
		&& pDph_t->symbols[n]  == USP_AX
		&& pDph_t->symbols[n+1]  == WBOUND)
	{
		pDph_t->symbols[n]  = USP_EY;
	}
	if (pDph_t->symbols[n-2]  >= GEN_SIL
		&& pDph_t->symbols[n-1]  >= WBOUND
		&& pDph_t->symbols[n]  == USP_AX
		&& pDph_t->symbols[n+1]  == WBOUND
		&& pDph_t->nsymbtot < 20 )
...
///reiner rule to compensate for bachus putting in a glottal stop in front of first 
		...
		if( pDph_t->symbols[n] == GRP_S
			&& pDph_t->symbols[n+1] == GRP_SH)
		{
			insertphone (phTTS, (short)(n+1), ( SPP_S));
		}
			...
			
		{
			...
			if((pDph_t->symbols[n] & PVALUE) >= WBOUND )
			{
				if (pDph_t->phrasal_verb==1)
				{
					pDph_t->phrasal_verb++;
				}
				if(pDph_t->phrasal_verb == 3 && word_start!=n)
				{
					pDph_t->phrasal_verb=0;
					word_end = n;	
					find_syll_to_stress (phTTS, &word_end, word_start);
				}
				if(pDph_t->phrasal_verb == 2)
				{
					pDph_t->phrasal_verb++;
					word_start = n;	
				}
			...
			if(pDph_t->symbols[n]== VPSTART )
			{
				pDph_t->phrasal_verb = 1;
			}
// for Raforzamento sintattico  
// This will self destruct on 12/25/04  MGS
// RESURRECTED  Reiner        07/27/05  REW  		
...
// consonant, there should be no doubling. Reiner.
// Modification: K doubles as KD followed by K, (Reiner 10/13/05)
// Modification proposed: Oct 13, 2005: Redublication can only occure if the first 
// consonant of the word is followed by a vowel or by l or r. (not for w )
// Further: Reduce the dublication duration: Don't double the duration, only 
// have an increase of about 50% of the total duration. E.g., if there is a K
// of duration 100 then the sequence KD - K should have a duration 
// KD <50> K <100>
// Reiner
				

		if (pDph_t->symbols[n] == ITP_RAF && 
			pDph_t->symbols[n+1]==WBOUND )
		{
			short temp=0;
			short tmp;
			for(tmp=n+2; tmp < pDph_t->nsymbtot 
				&& pDph_t->symbols[tmp] < 128; tmp++);
			switch(pDph_t->symbols[tmp])
			{
			case ITP_T:
				temp=ITP_TD;
				break;
...
// end of extra code to handle rafforzamente
...
if(pKsd_t->lang_curr == LANG_german)
{
		...
		if ((pDph_t->symbols[n - 1]& PVALUE) == S1
			&& pDph_t->symbols[n] == ITP_IY
			&& (pDph_t->symbols[n+1]& PVALUE)== WBOUND)
				{				
					delete_symbol (phTTS, n-1);
				}

		if ((pDph_t->symbols[n - 2] & PVALUE) >= WBOUND)
			if( ((pDph_t->symbols[n - 1]& PVALUE) == S1)
			|| ((pDph_t->symbols[n - 1]& PVALUE) == SEMPH))
				if ((pDph_t->symbols[n + 1] & PVALUE)>= WBOUND)
				if( pDph_t->symbols[n] == USP_AX)
			{
				pDph_t->symbols[n] = USP_EY;
			}
   ...
   || pKsd_t->lang_curr == LANG_spanish )
{
...

		{
			...
			{
				if( ((pKsd_t->lang_curr == LANG_english  || pKsd_t->lang_curr == LANG_french) && 
...
if (((pDph_t->symbols[n]& PVALUE) >= COMMA) && ((pDph_t->symbols[n]& PVALUE) <= EXCLAIM))
			...
			find_syll_to_stress (phTTS, &n, startword);	/* MVP : was n and n was static */
   ...
   || pKsd_t->lang_curr == LANG_latin_american)
{
			...
			if(pKsd_t->lang_curr == LANG_english  || pKsd_t->lang_curr == LANG_german)
			{
...
static void raise_last_stress (LPTTS_HANDLE_T phTTS, short msym)
{
	...
	//chnge to raising firat syl nuc of final word
	for (m ; m < msym; m++)
	{

		if (phone_feature((void *)phTTS,(pDph_t->symbols[m]& PVALUE)) & FSYLL)
		{
			insertphone (phTTS, (short)(m), ( S1));
			return;
		}
	}
...
short  *durlookup(PDPH_T pDph_t,short *symbol , short table[])
{
	...
	{            					/* 0 => end of table.   */
		...
		for (;;) 
		{
			if(!(*lp == 256 || (*lp >= 115 && *lp <= 118 )
				&& *cp == 111))
			{
				//force match on wbound equivalences
				if (*lp != *cp++)         /* Lose match process   */
					break;
			}
				else
					*cp++;
			if (*lp == GEN_SIL && flag ==1)         /* Input string end reached   */
				break;
			flag=1;
			if (*cp == EOS)           /* Win.                 */
            {
...
void add_feature (PDPH_T pDph_t, long feaname, short location)
{
	...
	if(feaname & FSTRESS && ((pDph_t->sentstruc[location] & FSTRESS) < (feaname & FSTRESS)))
		//if adding stress clear stress bits so we don't add them together
