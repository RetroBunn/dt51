	...
	else if (pDphsettar->par_type IS_AV_OR_AH)
	{
		...
		if (npar == AV - 1)
		{
				...
				&& ((phone_feature( pDph_t,phlas_temp) & FVOICD) IS_MINUS))
			{
				...
				/* devoice if previous a nasal */
			if (((phone_feature( pDph_t,phone_temp) & FSTOP) IS_MINUS)
				&& ((phone_feature( pDph_t,phone_temp) & FOBST) IS_PLUS)
				&& ((phone_feature( pDph_t,phone_temp) & FVOICD) IS_PLUS)
				&& ((phone_feature( pDph_t,phlas_temp) & FNASAL) IS_PLUS))
			{
				tartemp = 40;
			}
			 ...
			 || (pDphsettar->par_type IS_AV_OR_AH))
	{
		...
		if (pDphsettar->bouval < temp)
		{
			...
			//debugforward (" Offset", pDphsettar->np);
			 ...
			 if((pDphsettar->phcur == USP_Z || 
				pDphsettar->phcur == USP_V)
				&& ((feanex & FVOICD) IS_MINUS
				|| (feanex & FSTOP) IS_PLUS)
				&& pDphsettar->np == &PAV)
			 {
				 pDphsettar->durtran = NF70MS;
				pDphsettar->bouval -=  4;
				pDphsettar->np->tarcur= 20;
			 }
		...
		if (pDphsettar->np == &PAP)
		{
				...
				|| (pDphsettar->phcur == USP_S) || (pDphsettar->phcur == USP_SH))
			{
				if (((fealas & FVOICD) IS_PLUS)
					...
					&& (struclas & FDUMMY_VOWEL) IS_MINUS)
				{
					if (pDphsettar->phcur == GEN_SIL)
					{
...
*/
	...
	/* Don't release burst in homorganic plosiv-nasal & plos-plos sequence */
		...
		{
			if (us_place[pDphsettar->phcur] == us_place[pDphsettar->phonex])
			{						   /* Was >= */
				bdur = 0;
			}
		}
