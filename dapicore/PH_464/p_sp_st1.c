...
int sp_spec_coart (PDPH_T pDph_t,
 ...
 */
	 ...
	 */
	temp += sp_spec_coart (pDph_t,foncur, fonlas);
	temp += sp_spec_coart (pDph_t,foncur, fonnex);
...
//	PKSD_T                  pKsd_t = phTTS->pKernelShareData;
		 ...
		 && ((feacur & FSONOR) IS_PLUS)))
	{
		...
		if (pholas == SPP_K)
		{
			vot = 6;	   /* Vot before unstressed vowel  */
		}
		if (pholas == SPP_T)
		{
			vot = 3;	   /* Vot before unstressed vowel  */
		}
	...
	if ((feacur & FVBAR) IS_PLUS)
	{
		...
		PAV.pspesh = 40;  /* Amp of voicebar, AV target invalid   */
